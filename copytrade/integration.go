...
var (
	...
	_ trader.MarkPriceProvider              = (*trader.AutoTrader)(nil)
	_ trader.MarkPriceHistoryProvider       = (*trader.AutoTrader)(nil)
	_ trader.CopyGuardPositionModeValidator = (*trader.AutoTrader)(nil)
...
type TraderIntegration struct {
	lastExitEvidenceRecovery time.Time // execution reconciliation goroutine only
	lastExitEvidenceID       int64
	traderID                 string
	lifecycleGeneration      int64
	executor                 DecisionExecutor
	engine                   *Engine
	store                    *store.Store
	// policySnapshot is the credential-free canonical policy template loaded
	// from store. New cycles copy this immutable string; runtime CopyConfig is
	// never serialized into lifecycle storage.
	...
	exitExecutionMu sync.Mutex
	// protectionMu serializes venue-side protective-order maintenance with
	// foreground position transitions. The database also enforces one-way fixed
	// stops, but exchange amend/cancel/place sequences must not overlap inside
	// one process or an older refresh can retire a newer, tighter order.
	protectionMu    sync.Mutex
	protectionWake  chan struct{}
	protectionAsync atomic.Bool
	// riskExitMu is the in-process order boundary for a trusted Copy Guard
	// trigger. Decision execution holds a read lock only while calling the venue;
	// a trigger takes the write lock, installs the gate, then persists it. Thus an
	// order is either already submitted before the trigger or is deterministically
	// blocked after it—there is no check-then-submit race.
	riskExitMu    sync.RWMutex
	riskExitGates map[string]*riskExitGateState
	...
	// Also guards stopRiskAlerts, which is written from the same two goroutines.
	...
	// immediately fails verification" cycle keeps the counter near zero and the
	// backoff cap is never reached. Only touched from the
	// monitorV4ProtectiveStops goroutine, like protectiveQueryFailures.
	protectionRearms map[int64]*protectionRearmTracker
	// lastPositionMarginSafetyRecheck rate-limits the fresh mark/liquidation/
	// leverage sweep for healthy fixed-stop cycles. Position transitions still
	// revalidate synchronously; this timestamp covers changes that the venue can
	// make without a leader signal (most importantly a moving liquidation line).
	// It is touched only by monitorV4ProtectiveStops.
	lastPositionMarginSafetyRecheck time.Time
	shadowMarkMu                    sync.Mutex
	shadowMarks                     map[int64]*shadowMarkAccumulator
	cycleNumber                     int // 跟单周期计数器
}

type riskExitGateState struct {
...
type shadowMarkAccumulator struct {
	bucketAt         time.Time
...
func NewTraderIntegrationWithGeneration(
	...
	return &TraderIntegration{
		protectionWake:          make(chan struct{}, 1),
		traderID:                traderID,
		...
		riskExitGates:           make(map[string]*riskExitGateState),
...
func (ti *TraderIntegration) establishRiskExitGate(in store.CopyGuardRiskExitBegin) error {
	if ti == nil || ti.store == nil || in.CycleID <= 0 || in.LeaderPosID == "" {
		return fmt.Errorf("invalid in-memory Copy Guard risk exit gate")
	}
	in.Metadata = maps.Clone(in.Metadata)
	ti.riskExitMu.Lock()
	if ti.riskExitGates == nil {
		ti.riskExitGates = make(map[string]*riskExitGateState)
	}
	gate := ti.riskExitGates[in.LeaderPosID]
	if gate == nil || gate.begin.CycleID != in.CycleID || gate.begin.AttemptNo != in.AttemptNo {
		gate = &riskExitGateState{begin: in, createdAt: time.Now()}
		ti.riskExitGates[in.LeaderPosID] = gate
	} else {
		// Preserve the first trusted trigger identity while filling in richer
		// exchange evidence supplied by a later reconciliation pass.
		if gate.begin.TriggerPrice <= 0 && in.TriggerPrice > 0 {
			gate.begin.TriggerPrice = in.TriggerPrice
		}
		if gate.begin.Quantity <= 0 && in.Quantity > 0 {
			gate.begin.Quantity = in.Quantity
		}
		if gate.begin.AlgoID == "" && in.AlgoID != "" {
			gate.begin.AlgoID = in.AlgoID
		}
		if orderID := getStringField(in.Metadata, "actual_order_id"); orderID != "" && getStringField(gate.begin.Metadata, "actual_order_id") == "" {
			// The venue may expose the fill order only after the first trigger
			// acknowledgement. Copy-on-write keeps concurrent persistence reads
			// immutable while retaining that identity across restart.
			metadata := make(map[string]interface{}, len(gate.begin.Metadata)+1)
			for key, value := range gate.begin.Metadata {
				metadata[key] = value
			}
			metadata["actual_order_id"] = orderID
			gate.begin.Metadata = metadata
		}
	}
	begin := gate.begin
	// Store event normalization adds audit keys. It must never mutate the
	// shared in-memory gate or the caller's evidence map outside this lock.
	begin.Metadata = maps.Clone(gate.begin.Metadata)
	ti.riskExitMu.Unlock()

	_, err := ti.store.CopyTrade().BeginCopyGuardRiskExit(begin)
	if err == nil {
		err = ti.store.CopyTrade().BlockFollowGroupRiskByCycle(begin.CycleID)
	}
	ti.riskExitMu.Lock()
	if current := ti.riskExitGates[in.LeaderPosID]; current == gate {
		gate.persisted = err == nil
		if err != nil {
...
func (ti *TraderIntegration) retryRiskExitGatePersistence() {
	if ti == nil || ti.store == nil {
		return
	}
	ti.riskExitMu.RLock()
	pending := make([]store.CopyGuardRiskExitBegin, 0)
	for _, gate := range ti.riskExitGates {
		if gate != nil && !gate.persisted {
			pending = append(pending, gate.begin)
		}
	}
	ti.riskExitMu.RUnlock()
	for _, begin := range pending {
		if err := ti.establishRiskExitGate(begin); err != nil {
			logger.Errorf("❌ [%s] Copy Guard 风控退出 gate 持久化重试失败 | cycle=%d posId=%s: %v",
				ti.traderID, begin.CycleID, begin.LeaderPosID, err)
		}
	}
}

func (ti *TraderIntegration) hydrateRiskExitGates() {
	if ti == nil || ti.store == nil {
		return
	}
	...
	for _, cycle := range cycles {
		...
		switch cycle.Status {
		case store.CopyGuardStopPendingFlat, store.CopyGuardStopPartial, store.CopyGuardStopTriggered, store.CopyGuardStoppedWatching:
		default:
			continue
		}
		ti.riskExitGates[cycle.LeaderPosID] = &riskExitGateState{
			begin: store.CopyGuardRiskExitBegin{
				CycleID: cycle.ID, TraderID: ti.traderID, LeaderPosID: cycle.LeaderPosID,
				AttemptNo: cycle.ReentryCount, TriggerSource: "restart_recovery",
			},
			persisted: true, createdAt: time.Now(),
		}
	}
}

func (ti *TraderIntegration) riskExitGateActive(dec *decision.Decision) bool {
	if ti == nil || dec == nil || dec.LeaderPosID == "" {
		return false
	}
	ti.riskExitMu.RLock()
	gate := ti.matchingRiskExitGateLocked(dec)
	if gate != nil && ti.engine != nil && ti.engine.usesFollowGroups() {
		ti.riskExitMu.RUnlock()
		return true
	}
	if gate != nil && isAIReentryDecision(dec) {
		blocked := ti.riskExitGateBlocksReentryLocked(dec, gate)
		ti.riskExitMu.RUnlock()
		return blocked
	}
	ti.riskExitMu.RUnlock()
	...
	if errors.Is(err, sql.ErrNoRows) || (err == nil && cycle.ID != gate.begin.CycleID) {
		ti.riskExitMu.Lock()
		if current := ti.riskExitGates[dec.LeaderPosID]; current == gate {
			delete(ti.riskExitGates, dec.LeaderPosID)
		}
		ti.riskExitMu.Unlock()
		return false
	}
...
// gate. A queued reentry may pass only after the original exit is durably
// recorded and that exact lifecycle has explicitly advanced to
// REENTRY_PENDING. In particular, the in-memory-only window after a trusted
// trigger but before a successful database commit remains fail-closed.
//
// The caller holds riskExitMu.RLock so establishRiskExitGate cannot replace or
// downgrade the gate between this check and an exchange submission.
func (ti *TraderIntegration) riskExitGateBlocksReentryLocked(dec *decision.Decision, gate *riskExitGateState) bool {
	if ti == nil || dec == nil || gate == nil || !gate.persisted || ti.store == nil {
		return true
	}
	cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID)
	return err != nil || cycle.ID != gate.begin.CycleID || cycle.Status != store.CopyGuardReentryPending
}

func (ti *TraderIntegration) clearRiskExitGate(leaderPosID string, cycleID int64) {
	if ti == nil || leaderPosID == "" {
		return
	}
	ti.riskExitMu.Lock()
	if gate := ti.riskExitGates[leaderPosID]; gate != nil && (cycleID <= 0 || gate.begin.CycleID == cycleID) {
		delete(ti.riskExitGates, leaderPosID)
	}
	ti.riskExitMu.Unlock()
}

func (ti *TraderIntegration) executeDecisionUnderRiskExitGate(dec *decision.Decision, executionDecision *decision.Decision) error {
	ti.riskExitMu.RLock()
	if dec != nil {
		if gate := ti.matchingRiskExitGateLocked(dec); gate != nil {
			if !isAIReentryDecision(dec) || ti.riskExitGateBlocksReentryLocked(dec, gate) {
				ti.riskExitMu.RUnlock()
	...
	ti.riskExitMu.RUnlock()
	if ti.engine != nil && ti.engine.config != nil && ti.engine.config.FollowExitPolicyVersion >= 2 && isAIReentryDecision(dec) {
		return reasonError("CYCLE_REENTRY_DISABLED", "本轮风险退出后等待领航员下一轮开仓")
	}
	if err := ti.retireOldProtectionBeforeEntry(dec); err != nil {
		return reasonError("OLD_PROTECTION_RETIRING", "%v", err)
	}
	if err := ti.preflightCopyPositionOwnership(dec); err != nil {
		return err
	}
	// Do not hold the gate lock through price/leverage lookups or venue I/O.
	// The durable BeforeExchangeSubmit boundary below is the final check;
	// already-submitted orders are reconciled as in-flight residual exposure.
	return ti.executor.ExecuteDecision(executionDecision)
}

func (ti *TraderIntegration) recordRiskExitSignalIgnored(dec *decision.Decision) {
	if ti == nil || dec == nil || ti.store == nil || dec.LeaderPosID == "" {
		return
	}
	cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID)
	...
	_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
		CycleID: cycle.ID, TraderID: ti.traderID, Type: "RISK_EXIT_SIGNAL_IGNORED",
		Price: dec.EntryPrice, Quantity: dec.FilledQuantity, Notional: dec.PositionSizeUSD,
		Metadata: map[string]interface{}{
			"attempt_no": cycle.ReentryCount, "source_action": dec.Action,
			"copy_trade_action": dec.CopyTradeAction, "execution_intent_id": dec.ExecutionIntentID,
		},
	})
}
...
func (ti *TraderIntegration) StartCopyTrading() error {
	...
	if copyConfig.RiskPolicyVersion >= 4 {
		ti.policySnapshot, err = ti.store.CopyTrade().EncodeCopyGuardLifecyclePolicySnapshot(copyConfig)
		if err != nil {
			return fmt.Errorf("encode canonical Copy Guard lifecycle policy: %w", err)
		}
	}
	openCycles, err := ti.store.CopyTrade().ListOpenCopyGuardCycles(ti.traderID)
	if err != nil {
		return fmt.Errorf("load active Copy Guard lifecycles: %w", err)
	}
	hasActiveCopyGuardCycles := len(openCycles) > 0
	if SupportsCopyGuard(ProviderType(copyConfig.ProviderType)) &&
		(copyConfig.FollowExitPolicyVersion >= 2 || copyConfig.RiskStopLossEnabled || copyConfig.RiskLiquidationGuardEnabled || hasActiveCopyGuardCycles) {
		owner, ownerErr := ti.store.Trader().GetByID(ti.traderID)
		if ownerErr != nil {
			return fmt.Errorf("load execution account owner: %w", ownerErr)
		}
		if claimErr := ti.store.Trader().ClaimRunningCopyAccount(ti.traderID, ti.lifecycleGeneration, owner.ExchangeID); claimErr != nil {
			return fmt.Errorf("claim copy execution account: %w", claimErr)
		}
	}
	if SupportsCopyGuard(ProviderType(copyConfig.ProviderType)) && copyConfig.FollowExitPolicyVersion >= 2 {
		if err := validateLeaderExitCapabilities(ti.executor); err != nil {
			return fmt.Errorf("leader total-position exit unavailable: %w", err)
		}
	}
	if SupportsCopyGuard(ProviderType(copyConfig.ProviderType)) &&
		((copyConfig.RiskPolicyVersion >= 4 && (copyConfig.RiskStopLossEnabled || copyConfig.RiskLiquidationGuardEnabled)) || hasActiveCopyGuardCycles) {
	...
	engineConfig := &CopyConfig{
		...
		RiskLiquidationGuardEnabled:    copyConfig.RiskLiquidationGuardEnabled,
		FollowExitPolicyVersion:        copyConfig.FollowExitPolicyVersion,
		RiskProtectionMode:             copyConfig.RiskProtectionMode,
		RiskPositionMarginStopPct:      copyConfig.RiskPositionMarginStopPct,
	...
	if SupportsCopyGuard(engineConfig.ProviderType) && ((engineConfig.RiskPolicyVersion >= 4 && (engineConfig.RiskStopLossEnabled || engineConfig.RiskLiquidationGuardEnabled)) || hasActiveCopyGuardCycles) {
	...
	// Enable durable protection dispatch before decisions can be consumed.
	ti.protectionAsync.Store(SupportsCopyGuard(engineConfig.ProviderType) && (engineConfig.RiskPolicyVersion >= 4 || hasActiveCopyGuardCycles))
	// 启动决策消费协程
	...
	if SupportsCopyGuard(engineConfig.ProviderType) && (engineConfig.RiskPolicyVersion >= 4 || hasActiveCopyGuardCycles) {
		ti.recoverV4PendingStates()
...
func (ti *TraderIntegration) repriceActiveProtections() {
	...
	for _, mapping := range mappings {
		...
		ti.queueProtectionRefresh(&decision.Decision{
...
func (ti *TraderIntegration) reconcileExecutionIntents(startup bool) {
	...
	if startup || time.Since(ti.lastExitEvidenceRecovery) >= time.Minute {
		ti.lastExitEvidenceRecovery = time.Now()
	...
	for _, intent := range intents {
		if scope, ratio, readErr := ti.store.CopyTrade().LeaderExitRequest(intent.ID); readErr == nil && scope == leaderAccountExitScope {
			dec := &decision.Decision{IsCopyTrade: true, LeaderExitScope: scope, CloseRatio: ratio, ExecutionIntentID: intent.ID, SourceRevision: intent.SourceRevision, LeaderPosID: intent.LeaderPosID, LeaderPosSize: intent.LeaderTargetSize, Symbol: intent.Symbol, Action: intent.Action, CopyTradeAction: "reduce", SourceFillID: intent.SourceFillID, ClientOrderID: intent.ClientOrderID, LeaderReversed: sourceFillMarksLeaderReversal(intent.SourceFillID)}
			if strings.HasPrefix(intent.Action, "close_") {
				dec.CopyTradeAction = "close"
			}
			ti.executionMu.Lock()
			err := ti.executeAccountLeaderExit(dec, !startup)
			ti.executionMu.Unlock()
			if err != nil {
				ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "LEADER_EXIT_PENDING", err.Error())
			}
			continue
		}
		if intent.SourceKind == copyGuardRiskExitSource {
			if err := ti.reconcileRiskExitIntent(intent); err != nil {
				logger.Warnf("[%s] risk exit acknowledgement pending: %v", ti.traderID, err)
			}
			continue
		}
		...
		if intent.Status == store.ExecutionIntentPartiallyFilled {
			...
			if len(attempts) == 0 || (attempts[len(attempts)-1].TerminalAt != nil && !store.ExecutionOrderAttemptNeedsReconciliation(attempts[len(attempts)-1])) {
				if !startup {
		...
		if intent.SourceKind == "LEADER_TRANSITION" && intent.FilledQuantity > 0 {
			resolved, resolveErr := ti.store.CopyTrade().ResolveAcknowledgedLeaderIntent(intent.ID)
			if resolveErr != nil {
				ti.recordExecutionReconciliationFailure(intent, "SOURCE_FINISH_PENDING", resolveErr.Error())
				continue
			}
			if resolved {
		...
		if mapping, mappingErr := ti.store.CopyTrade().GetMappingForReconciliation(ti.traderID, intent.LeaderPosID); mappingErr == nil && mapping != nil && mapping.SourceRevision > intent.SourceRevision {
			ti.recordExecutionReconciliationFailure(intent, "HISTORICAL_FILL_EVIDENCE_PENDING",
				fmt.Sprintf("mapping revision %d advanced beyond intent %d; immutable per-order booking evidence remains incomplete", mapping.SourceRevision, intent.SourceRevision))
			continue
		}
		...
		ti.observeExecutionFillTime(intent.ID, order)
		...
		if filled <= 0 {
			...
			if terminal && nextIntentStatus == store.ExecutionIntentFailed {
				ti.acknowledgeTerminalZeroFill(intent)
			}
	...
	if intent.CandidateID > 0 {
		if candidate, candidateErr := rs.GetReentryCandidate(intent.CandidateID); candidateErr == nil {
			...
			if cycle, cycleErr := ti.store.CopyTrade().GetCopyGuardCycle(intent.CycleID); cycleErr == nil {
				cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
				if attempts, attemptsErr := ti.store.CopyTrade().ListCopyGuardAttempts(cycle.ID); attemptsErr == nil {
					...
					if cycleConfig != nil {
						dec.AIPlannedNotional = stopped * cycleConfig.RiskReentryRatio * candidate.SizeFactor
						dec.AIConfiguredMinimum = cycleConfig.RiskReentryMinNotional
					}
...
func validateV4ExecutorCapabilities(executor DecisionExecutor) error {
	checks := []struct {
		...
		{"live position-mode verification", implements[trader.CopyGuardPositionModeValidator](executor)},
	}
	...
	validator := executor.(trader.CopyGuardPositionModeValidator)
	if err := validator.ValidateCopyGuardPositionMode(); err != nil {
		return fmt.Errorf("unsafe Copy Guard account position mode: %w", err)
	}
	return nil
}

func validateLeaderExitCapabilities(executor DecisionExecutor) error {
	if !implements[trader.ScopedPositionCloser](executor) || !implements[FreshPositionsProvider](executor) ||
		!implements[ClientOrderStatusProvider](executor) || !implements[trader.ExecutionInstrumentResolver](executor) {
		return fmt.Errorf("executor requires scoped closes, fresh positions, order lookup and quantity precision")
	}
	if validator, ok := executor.(interface{ ValidateLeaderExitCapabilities() error }); ok {
		return validator.ValidateLeaderExitCapabilities()
	}
	return nil
}
...
func (ti *TraderIntegration) monitorV4ProtectiveStops() {
	...
	for {
		...
		case <-ti.ctx.Done():
			return
		case <-ti.protectionWake:
		case <-ticker.C:
		}
		ti.drainProtectionRefreshes()
		ti.retryCompletedLeaderExitProtections()
		ti.retryAcknowledgedCloseProtections()
		ti.retryRiskExitGatePersistence()
		ti.retryPendingAIEntryLeases()
		persisted, configErr := ti.store.CopyTrade().GetByTraderID(ti.traderID)
		if configErr != nil {
			// Active lifecycle protection is snapshot-owned and must continue
			// even if the mutable trader template is temporarily unavailable.
			logger.Errorf("❌ [%s] Copy Guard 配置读取失败；仅暂停新周期回填，现有周期继续巡检: %v", ti.traderID, configErr)
		} else if persisted.RiskStopLossEnabled || persisted.RiskLiquidationGuardEnabled {
			ti.backfillV4Cycles()
		}
		ti.pollV4ProtectiveStops()
...
const positionMarginSafetyRecheckInterval = 15 * time.Second

// recheckPositionMarginSafety refreshes the venue facts that can invalidate a
// healthy fixed stop without any leader position event. It deliberately shares
// the normal protection planner: the immutable anchor, durable one-way clamp,
// safe replacement and crossed-stop exit all keep one implementation. A single
// fresh account snapshot is reused for every due cycle to avoid multiplying
// exchange traffic by the number of open positions.
func (ti *TraderIntegration) recheckPositionMarginSafety() {
	if ti == nil || ti.store == nil || ti.engine == nil || ti.engine.config == nil {
		return
	}
	now := time.Now()
	if !ti.lastPositionMarginSafetyRecheck.IsZero() && now.Sub(ti.lastPositionMarginSafetyRecheck) < positionMarginSafetyRecheckInterval {
		return
	}
	ti.lastPositionMarginSafetyRecheck = now

	cycles, err := ti.store.CopyTrade().ListOpenCopyGuardCycles(ti.traderID)
	if err != nil {
		return
	}
	fixedCycles := make([]*store.CopyGuardCycle, 0, len(cycles))
	for _, cycle := range cycles {
		if cycle == nil || (cycle.Status != store.CopyGuardFollowing && cycle.Status != store.CopyGuardFollowingReentry) {
			continue
		}
		mode, _ := copyGuardProtectionSettings(cycle, ti.engine.config)
		if mode == store.RiskProtectionModePositionMarginPct || copyGuardLifecycleConfig(cycle, ti.engine.config).RiskLiquidationGuardEnabled {
			fixedCycles = append(fixedCycles, cycle)
		}
	}
	if len(fixedCycles) == 0 {
		return
	}

	ti.protectionMu.Lock()
	defer ti.protectionMu.Unlock()
	positions, positionsErr := ti.getFreshPositions()
	if positionsErr != nil {
		for _, cycle := range fixedCycles {
			ti.recordPositionMarginMarkUnavailable(cycle, cycle.Symbol, "periodic_safety_snapshot", positionsErr)
		}
		return
	}
	for _, cycle := range fixedCycles {
		...
		ti.refreshStopLossAfterExecuteLocked(&decision.Decision{
			Symbol: cycle.Symbol, Action: action, IsCopyTrade: true,
			CopyTradeAction: "protection_safety_recheck", LeaderPosID: cycle.LeaderPosID,
			MarginMode: cycle.MarginMode, EntryPrice: cycle.FollowerEntryPrice,
			Reasoning: "Copy Guard fixed-stop periodic safety recheck",
		}, positions, nil)
	}
}
...
func (ti *TraderIntegration) retryPendingAIEntryLeases() {
	...
	for _, candidate := range candidates {
		...
		retrySeconds := ti.engine.config.RiskAIMinReviewSeconds
		if cycle, cycleErr := ti.store.CopyTrade().GetCopyGuardCycle(candidate.CycleID); cycleErr == nil {
			if cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config); cycleConfig != nil {
				retrySeconds = cycleConfig.RiskAIMinReviewSeconds
			}
		}
		_ = ti.store.ReentryAI().RejectReentryCandidatePreflight(candidate.ID, err.Error(), time.Duration(retrySeconds)*time.Second)
	}
}
...
func (ti *TraderIntegration) ensureV4CycleForMapping(mapping *store.CopyTradePositionMapping, positions []map[string]interface{}, reason string) (*store.CopyGuardCycle, bool, error) {
	...
	if !SupportsCopyGuard(cfg.ProviderType) || cfg.RiskPolicyVersion < 4 || !(cfg.RiskStopLossEnabled || cfg.RiskLiquidationGuardEnabled) {
		return nil, false, nil
	}
	if cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, mapping.LeaderPosID); err == nil {
		if bindErr := ti.store.CopyTrade().BindFollowGroupGuard(cycle.ID); bindErr != nil {
	...
	if cycle, err := ti.store.CopyTrade().GetFollowGroupProtectionCycle(ti.traderID, mapping.LeaderPosID, mapping.MarginMode); err == nil {
		return cycle, false, nil
	} else if !errors.Is(err, sql.ErrNoRows) {
		return nil, false, err
	}
	initialIntent, initialIntentAvailable, err := ti.store.CopyTrade().GetConfirmedInitialLeaderExecutionIntent(ti.traderID, mapping.LeaderPosID)
	if err != nil {
		return nil, false, fmt.Errorf("load confirmed initial follower fill: %w", err)
	}
	// A missing lifecycle does not grant ownership over today's same-side
	// position. Prove continuity before backfilling historical protection.
	unownedNet := float64(-1)
	if custody, readErr := ti.store.CopyTrade().GetPositionCustody(ti.traderID, mapping.LeaderPosID); readErr == nil {
		if custody.State == "RELEASED" {
			return nil, false, errCopyPositionEnded
		}
		if _, historyAvailable := ti.executor.(trader.SymbolTradeHistoryProvider); historyAvailable {
			if _, continuityErr := ti.verifyFollowingPosition(mapping, custody); continuityErr != nil {
				return nil, false, continuityErr
			}
		}
	} else if !errors.Is(readErr, sql.ErrNoRows) {
		return nil, false, readErr
	} else {
		// Legacy rows may predate custody. Only an identified, confirmed entry
		// and complete margin-scoped history can establish continuity. A current
		// same-side snapshot alone could belong to a later manual position.
		if !initialIntentAvailable || initialIntent.Symbol != mapping.Symbol ||
			initialIntent.Action != "open_"+strings.ToLower(mapping.Side) ||
			initialIntent.MarginMode == "" || initialIntent.MarginMode != mapping.MarginMode ||
			initialIntent.SourceRevision > mapping.SourceRevision {
			return nil, false, fmt.Errorf("legacy custody requires confirmed initial fill identity for the exact position scope")
		}
		history, ok := ti.executor.(trader.ScopedTradeHistoryProvider)
		if !ok {
			return nil, false, fmt.Errorf("legacy custody requires complete margin-scoped fill history")
		}
		attempts, readErr := ti.store.CopyTrade().ListExecutionOrderAttempts(initialIntent.ID)
		if readErr != nil {
			return nil, false, readErr
		}
		expected := map[string]float64{}
		entryID, expectedTotal := "", float64(0)
		for _, attempt := range attempts {
			if store.ExecutionOrderAttemptNeedsReconciliation(attempt) {
				return nil, false, fmt.Errorf("initial follower order remains unresolved")
			}
			if attempt.FilledQuantity <= 0 {
				continue
			}
			if attempt.ExchangeOrderID == "" {
				return nil, false, fmt.Errorf("initial follower order identity is missing")
			}
			if entryID == "" {
				entryID = attempt.ExchangeOrderID
			}
			expected[attempt.ExchangeOrderID] += attempt.FilledQuantity
			expectedTotal += attempt.FilledQuantity
		}
		if len(attempts) == 0 && initialIntent.ExchangeOrderID != "" {
			entryID, expectedTotal = initialIntent.ExchangeOrderID, initialIntent.FilledQuantity
			expected[entryID] = expectedTotal
		}
		if entryID == "" || expectedTotal <= 0 || math.IsNaN(expectedTotal) || math.IsInf(expectedTotal, 0) ||
			math.Abs(expectedTotal-initialIntent.FilledQuantity) > math.Max(1e-12, initialIntent.FilledQuantity*1e-9) {
			return nil, false, fmt.Errorf("initial follower fill quantity is not fully identified")
		}
		fills, readErr := history.GetTradesForPosition(mapping.Symbol, mapping.MarginMode, initialIntent.CreatedAt.Add(-2*time.Second))
		if readErr != nil {
			return nil, false, fmt.Errorf("legacy custody history unavailable: %w", readErr)
		}
		confirmed, seen, actions := map[string]float64{}, map[string]trader.TradeRecord{}, map[int64]string{}
		var scoped []trader.TradeRecord
		started := false
		unownedNet = 0
		for _, fill := range orderedContinuityFills(fills) {
			if fill.Symbol != mapping.Symbol || (!strings.EqualFold(fill.PositionSide, mapping.Side) && !strings.EqualFold(fill.PositionSide, "BOTH")) {
				continue
			}
			if fill.TradeID == "" || fill.OrderID == "" || fill.Time.IsZero() || fill.MarginMode != mapping.MarginMode ||
				fill.Quantity <= 0 || math.IsNaN(fill.Quantity) || math.IsInf(fill.Quantity, 0) ||
				(!strings.EqualFold(fill.Side, "BUY") && !strings.EqualFold(fill.Side, "SELL")) {
				return nil, false, fmt.Errorf("legacy custody fill identity or scope is incomplete")
			}
			if prior, exists := seen[fill.TradeID]; exists {
				if prior.OrderID != fill.OrderID || prior.Side != fill.Side || prior.Quantity != fill.Quantity || !prior.Time.Equal(fill.Time) {
					return nil, false, fmt.Errorf("legacy custody duplicate fill identity conflicts")
				}
				continue
			}
			seen[fill.TradeID] = fill
			if !started && fill.OrderID == entryID {
				started = true
			}
			if !started {
				continue
			}
			if previous := actions[fill.Time.UnixMilli()]; previous != "" && previous != strings.ToUpper(fill.Side) {
				return nil, false, fmt.Errorf("legacy custody has ambiguous same-millisecond entry/exit order")
			}
			actions[fill.Time.UnixMilli()] = strings.ToUpper(fill.Side)
			increase := strings.EqualFold(fill.Side, "BUY") == strings.EqualFold(mapping.Side, "long")
			if _, isInitial := expected[fill.OrderID]; isInitial {
				if !increase {
					return nil, false, fmt.Errorf("initial follower order has an incompatible fill direction")
				}
				confirmed[fill.OrderID] += fill.Quantity
			}
			if increase {
				unownedNet += fill.Quantity
			} else {
				unownedNet -= fill.Quantity
			}
			scoped = append(scoped, fill)
		}
		for id, quantity := range expected {
			if math.Abs(confirmed[id]-quantity) > math.Max(1e-12, quantity*1e-9) {
				return nil, false, fmt.Errorf("initial follower order fills are incomplete")
			}
		}
		if ended, continuityErr := positionEndedInFills(scoped, entryID, mapping.Symbol, mapping.Side); continuityErr != nil {
			return nil, false, continuityErr
		} else if ended {
			return nil, false, errCopyPositionEnded
		}
		// The mutable intent order id may point at its last catch-up order;
		// freeze the first proved entry for future continuity checks.
		initialIntent.ExchangeOrderID = entryID
		positions = nil // Compare the proved remaining quantity with a fresh read.
	}
	if positions == nil {
		var err error
	...
	for _, pos := range positions {
		...
		mode := getStringField(pos, "mgnMode", "marginMode")
		if unownedNet >= 0 && mode != mapping.MarginMode {
			continue
		}
		if mapping.MarginMode != "" && mode != "" && mode != mapping.MarginMode {
			continue
		}
		if qty > 0 && unownedNet >= 0 {
			return nil, false, fmt.Errorf("legacy custody has multiple indistinguishable follower positions")
		}
	...
	if qty <= 0 || entryPrice <= 0 || math.IsNaN(qty) || math.IsInf(qty, 0) || math.IsNaN(entryPrice) || math.IsInf(entryPrice, 0) {
		return nil, false, fmt.Errorf("fresh follower position is unavailable for %s %s", mapping.Symbol, mapping.Side)
	}
	if unownedNet >= 0 && math.Abs(unownedNet-qty) > math.Max(1e-12, qty*1e-9) {
		return nil, false, fmt.Errorf("legacy custody fill history does not reconcile to the current position")
	}
	leaderEntry := mapping.OpenPrice
	if leaderEntry <= 0 {
		leaderEntry = entryPrice
	}
	policyJSON := ti.policySnapshot
	if strings.TrimSpace(policyJSON) == "" {
		return nil, false, fmt.Errorf("canonical copy guard policy snapshot is unavailable")
	}
	policy, err := store.DecodeCopyGuardPolicySnapshot(policyJSON)
	if err != nil {
		return nil, false, fmt.Errorf("decode canonical Copy Guard recovery policy: %w", err)
	}
	if strings.TrimSpace(reason) == "" {
		reason = "active mapping lifecycle recovery"
	}
	// A missing fixed-mode cycle can never safely recover its initial leverage:
	// the current position leverage may already have changed. New fixed opens are
	// atomic with their lifecycle, so such a row is necessarily legacy and must
	// not manufacture an 80% anchor from today's weighted average.
	if initialIntentAvailable && policy.ProtectionMode != store.RiskProtectionModePositionMarginPct {
		initialEntry := initialIntent.FilledNotional / initialIntent.FilledQuantity
		if initialEntry > 0 && !math.IsNaN(initialEntry) && !math.IsInf(initialEntry, 0) {
			cycle, created, ensureErr := ti.store.CopyTrade().EnsureInitialCopyGuardLifecycle(store.InitialCopyGuardLifecycle{
				IntentID: initialIntent.ID, TraderID: ti.traderID, LeaderID: cfg.LeaderID,
				LeaderPosID: mapping.LeaderPosID, Symbol: mapping.Symbol, Side: mapping.Side,
				MarginMode: mapping.MarginMode, PolicySnapshot: policyJSON,
				LeaderEntryPrice: leaderEntry, FollowerEntry: initialEntry,
				FollowerNotional: initialIntent.FilledNotional, FollowerQuantity: initialIntent.FilledQuantity,
				LeaderSize: initialIntent.LeaderTargetSize, AccountEquity: ti.getEquityFunc()(),
				ATRAtEntry: ti.copyGuardEntryATR(mapping.Symbol, initialEntry), EntryOrderID: initialIntent.ExchangeOrderID,
			})
			if ensureErr != nil {
				return nil, false, ensureErr
			}
			if updateErr := ti.store.CopyTrade().UpdateCopyGuardAttemptPosition(cycle.ID, 0, entryPrice, entryPrice*qty, qty, 0); updateErr != nil {
				return nil, false, fmt.Errorf("update recovered Copy Guard position: %w", updateErr)
			}
			if created {
				_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
					CycleID: cycle.ID, TraderID: ti.traderID, Type: "CYCLE_RECOVERED_FROM_INITIAL_FILL",
					Price: initialEntry, Quantity: initialIntent.FilledQuantity, Notional: initialIntent.FilledNotional,
					Metadata: map[string]interface{}{"reason": reason, "initial_intent_id": initialIntent.ID},
				})
			}
			if bindErr := ti.store.CopyTrade().BindFollowGroupGuard(cycle.ID); bindErr != nil {
				return nil, false, bindErr
	...
	legacyReason := reason + "; confirmed initial fill/leverage evidence is insufficient"
	cycle, created, err := ti.store.CopyTrade().EnsureLegacyUnverifiedCopyGuardLifecycle(store.LegacyUnverifiedCopyGuardLifecycle{
		...
		PolicySnapshot: policyJSON, LeaderEntryPrice: leaderEntry, FollowerEntry: entryPrice,
		FollowerNotional: entryPrice * qty, FollowerQuantity: qty, LeaderSize: mapping.LastKnownSize,
	...
	if created {
		logger.Errorf("🚨 [%s] Copy Guard 历史仓位缺少可信首仓成交证据 | cycle=%d posId=%s %s %s；继续跟随领航员，但禁止 fixed 锚点和影子评分直到本生命周期结束",
			ti.traderID, cycle.ID, mapping.LeaderPosID, mapping.Symbol, mapping.Side)
	}
	if bindErr := ti.store.CopyTrade().BindFollowGroupGuard(cycle.ID); bindErr != nil {
		return nil, false, bindErr
	}
...
func (ti *TraderIntegration) backfillV4Cycles() {
	...
	for _, mapping := range mappings {
		...
		if cerr != nil {
			ti.recordProtectionFollowup(&decision.Decision{LeaderPosID: mapping.LeaderPosID, Symbol: mapping.Symbol, Action: "open_" + mapping.Side}, "COPY_GUARD_CYCLE_RECOVERY_PENDING", cerr)
			continue
		}
		_ = ti.store.CopyTrade().ResolveRuntimeIssue(ti.traderID, "protection", mapping.LeaderPosID)
		if created {
...
func (ti *TraderIntegration) notifyCycleClosedSummary(cycle *store.CopyGuardCycle) {
	...
	if policyErr != nil {
		logger.Warnf("⚠️ [%s] Copy Guard 周期汇总策略快照不可解析 cycle=%d: %v", ti.traderID, cycle.ID, policyErr)
		return
	}
...
func (ti *TraderIntegration) reconcileV4CycleAccounting(cycle *store.CopyGuardCycle) {
	...
	if err := ti.verifyCopyGuardSettlement(cycle, cycle.ReentryCount, matched); err != nil {
		markFailure(err.Error())
		return
	}
...
func (ti *TraderIntegration) reconcileStoppedV4Attempts() {
	...
	for _, cycle := range cycles {
		...
		for _, attempt := range attempts {
			...
			if err := ti.verifyCopyGuardSettlement(cycle, attempt.AttemptNo, best); err != nil {
				continue
			}
...
func (ti *TraderIntegration) pollV4ProtectiveStops() {
	...
	for _, stored := range orders {
		...
		pendingExit := store.CopyGuardRiskExitPending(cycle.Status)
		if !ti.copyGuardOwnsPosition(cycle) && !pendingExit {
		...
		if ti.copyGuardOwnsPosition(cycle) {
			if continuityErr := ti.verifyCopyGuardContinuity(cycle); continuityErr != nil && !errors.Is(continuityErr, errCopyPositionEnded) {
				ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "POSITION_CONTINUITY_UNKNOWN", continuityErr, cycle.ProtectionCoverage, false)
				continue
			}
		}
		qty, known := ti.copyGuardFollowerQuantity(cycle, true)
		if known && qty > 0 {
			if err := ti.reconcileManualStop(cycle, qty, stored.QuantityStep); err != nil && !errors.Is(err, errCopyGuardRiskExitGate) {
				ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "MANUAL_STOP_AMBIGUOUS", err, cycle.ProtectionCoverage, false)
				continue
			}
			// A unique manual replacement may have changed the tracked identity.
			if current, readErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID); readErr == nil {
				stored = current
			}
		}
		...
		if live == nil {
			// Order disappearance is not stop evidence. Freshly confirmed flat
			// positions remain an open, detached lifecycle until the leader exits.
			_ = ti.store.CopyTrade().UpdateCopyGuardProtectiveOrderStatus(stored.CycleID, "invalid")
			if ti.isFollowerPositionFlat(cycle.Symbol, cycle.Side, cycle.MarginMode, cycle.FollowerPosID) {
				ti.confirmCopyGuardFollowerAbsent(cycle)
				continue
			}
		...
		if isProtectiveStopFired(state) {
			ti.observeTrustedProtectiveTrigger(cycle, stored, live)
		} else if !ti.copyGuardOwnsPosition(cycle) {
			_ = ti.cancelProtectiveOrderForCycle(mgr, cycle, stored)
			continue
		}
		observedCoverageMode := live.CoverageMode
		if observedCoverageMode == "" {
			observedCoverageMode = stored.CoverageMode
		}
		if err := ti.store.CopyTrade().ObserveCopyGuardProtectiveOrder(&store.CopyGuardProtectiveOrder{CycleID: cycle.ID, AlgoID: stored.AlgoID, Symbol: live.Symbol, Side: live.PositionSide, MarginMode: live.MarginMode, Quantity: live.Quantity, TriggerPrice: live.TriggerPrice, TriggerType: live.TriggerType, CoverageMode: observedCoverageMode}); err != nil {
			continue
		}
		...
		posQty, posAttributed := ti.copyGuardFollowerQuantity(cycle, false)
		if known {
			posQty, posAttributed = qty, true
		}
		if posAttributed && posQty <= 0 && !isProtectiveStopFired(state) && ti.confirmCopyGuardFollowerAbsent(cycle) {
			// Even while the exit's accounting is pending, a flat position no
			// longer needs this stop. Retire its exact identity; failures leave
			// the live order visible for the next poll and the new-entry fence.
			if err := ti.cancelProtectiveOrderForCycle(mgr, cycle, stored); err != nil {
				logger.Warnf("[%s] flat position protective order cleanup pending: cycle=%d err=%v", ti.traderID, cycle.ID, err)
			}
			continue
		}
		if posAttributed && (posQty > 0 || state == "live") {
			// A verified zero while the exchange still reports the protective
			// order as live is not a stop fill. Include it in the ownership
			// audit so an external/manual close cannot be promoted into a
			// strategy result or mistaken for a reentry-eligible risk stop.
			ti.auditCopyGuardPositionOwnership(cycle, posQty, stored.QuantityStep)
		}
		if posAttributed && posQty > 0 {
			baseQty = posQty
		} else if !posAttributed {
			// The follower position could not be attributed to this cycle, so there
			// is no trustworthy coverage baseline. Keep the locally stored quantity
			// (below) instead of an account-wide sum: judging coverage against a
			// number that belongs to another cycle can never reconcile, and every
			// poll would feed the re-arm chain. Surface it as a throttled diagnostic
			// that stays out of the retry chain.
			if ti.shouldRecordProtectionEvent(cycle.ID, "PROTECTION_COVERAGE_UNATTRIBUTED", cycle.MarginMode) {
				_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "PROTECTION_COVERAGE_UNATTRIBUTED", Quantity: stored.Quantity, Metadata: map[string]interface{}{"symbol": cycle.Symbol, "side": cycle.Side, "margin_mode": cycle.MarginMode, "follower_pos_id": cycle.FollowerPosID, "fallback_baseline": stored.Quantity}})
			}
		}
		liveCoverageMode := live.CoverageMode
		if liveCoverageMode == "" {
			liveCoverageMode = stored.CoverageMode
		}
		coverage := protectiveOrderCoverage(liveCoverageMode, live.Quantity, baseQty)
		if state == "live" {
			cfg := copyGuardLifecycleConfig(cycle, ti.engine.config)
			if cfg == nil {
				ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_POLICY_UNAVAILABLE", fmt.Errorf("lifecycle policy unavailable"), coverage, false)
				continue
			}
			target := stored.TriggerPrice
			attempts, readErr := ti.store.CopyTrade().ListCopyGuardAttempts(cycle.ID)
			if readErr != nil {
				continue
			}
			for _, attempt := range attempts {
				if attempt.AttemptNo == cycle.ReentryCount && attempt.FinalStopPrice > 0 {
					target = attempt.FinalStopPrice
				}
			}
			if !protectiveTriggerMatches(live, cycle.Symbol, cfg.RiskTriggerPriceType) ||
				!strings.EqualFold(live.PositionSide, cycle.Side) ||
				!protectiveMarginScopeMatches(live, cycle.MarginMode) ||
				liveCoverageMode != stored.CoverageMode || !sameProtectivePrice(live.TriggerPrice, target, 0) {
				ti.markProtectionIssue(cycle, store.CopyGuardProtectionDegraded, "PROTECTION_IDENTITY_MISMATCH", fmt.Errorf("venue protection identity, trigger type or price differs from lifecycle target"), coverage, false)
				continue
			}
			if !protectiveOrderQuantityMatches(liveCoverageMode, live.Quantity, baseQty, stored.QuantityStep) {
			...
			if protectiveOrderQuantityMatches(liveCoverageMode, live.Quantity, baseQty, stored.QuantityStep) {
				ti.notifyProtectionForEvent(cycle, "Copy Guard 保护已恢复", "保护单已经重新验证有效。", "recovered", "PROTECTION_RECOVERED")
			}
			...
			cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
			if protectiveOrderQuantityMatches(liveCoverageMode, live.Quantity, baseQty, stored.QuantityStep) && cycleConfig != nil && cycleConfig.RiskReentryDecisionMode == "ai_guarded" && cycle.Status == store.CopyGuardFollowingReentry {
		...
		ti.observeTrustedProtectiveTrigger(cycle, stored, live)
		mapping, mappingErr := ti.store.CopyTrade().GetMapping(ti.traderID, cycle.LeaderPosID)
		if mappingErr != nil && !errors.Is(mappingErr, sql.ErrNoRows) {
			logger.Errorf("[CopyGuard] trader=%s cycle=%d event=STOP_MAPPING_READ_FAILED reason=%v", ti.traderID, cycle.ID, mappingErr)
			continue
		}
		addCount := 0
		if mapping != nil {
		...
		leaderPnL, leaderSize := float64(0), float64(0)
		leader := ti.engine.buildLeaderPosMap()[cycle.LeaderPosID]
		if leader != nil {
			leaderPnL, leaderSize = leader.UnrealizedPnL, leader.Size
		}
		begin := store.CopyGuardRiskExitBegin{
			CycleID: cycle.ID, TraderID: ti.traderID, LeaderPosID: cycle.LeaderPosID,
			AttemptNo: cycle.ReentryCount, TriggerPrice: stored.TriggerPrice, Quantity: stored.Quantity,
			AlgoID: stored.AlgoID, TriggerSource: "exchange_hosted", LeaderPnL: leaderPnL,
			LeaderSize: leaderSize, AddCount: addCount,
			Metadata: map[string]interface{}{"state": state, "actual_order_id": live.ActualOrderID},
		}
		if beginErr := ti.establishRiskExitGate(begin); beginErr != nil {
			logger.Errorf("[CopyGuard] trader=%s cycle=%d attempt=%d event=STOP_BEGIN_FAILED reason=%v", ti.traderID, cycle.ID, cycle.ReentryCount, beginErr)
			continue
		}
		...
		remain, exists := ti.copyGuardFollowerQuantity(cycle, true)
		if !exists {
		...
		if remain > 0 {
			...
			partialChanged, partialErr := ti.store.CopyTrade().MarkCopyGuardRiskExitPartial(cycle.ID, cycle.ReentryCount, remain, stored.TriggerPrice,
				map[string]interface{}{"algo_id": stored.AlgoID, "state": state, "classification": eventType, "notional": notional})
			if partialErr != nil {
				logger.Errorf("[CopyGuard] trader=%s cycle=%d event=STOP_PARTIAL_PERSIST_FAILED reason=%v", ti.traderID, cycle.ID, partialErr)
				continue
			}
			if partialChanged {
			...
			if _, ok := ti.executor.(EmergencyPositionCloser); ok && ti.residualCloseTries[cycle.ID] < 3 && (ti.residualCloseLast[cycle.ID].IsZero() || time.Since(ti.residualCloseLast[cycle.ID]) >= 10*time.Second) {
				ti.residualCloseLast[cycle.ID] = time.Now()
				ti.residualCloseTries[cycle.ID]++
				if orderID, closeErr := ti.closeCopyGuardPosition(cycle); closeErr != nil {
				...
				ti.residualCloseLast[cycle.ID] = time.Now()
				if _, ok := ti.executor.(EmergencyPositionCloser); ok {
					orderID, closeErr := ti.closeCopyGuardPosition(cycle)
					if closeErr != nil {
		...
		cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
		atr := float64(0)
		if cycleConfig != nil && cycleConfig.RiskProtectionMode != store.RiskProtectionModePositionMarginPct {
			atr, _ = market.GetOKXATRWithMaxAge(cycle.Symbol, cycleConfig.RiskATRTimeframe, cycleConfig.RiskATRPeriod, riskATRCacheMaxAge(cycleConfig))
		}
		...
		finalize := store.CopyGuardRiskExitFinalize{
			CopyGuardRiskExitBegin: begin, ATR: atr, ExitPrice: fillPrice, PnL: stopPnL,
			Fee: fee, Slippage: slippage, ActualOrderID: live.ActualOrderID, VenueState: state,
		}
		if _, recordErr := ti.store.CopyTrade().FinalizeCopyGuardRiskExit(finalize); recordErr != nil {
		...
		if mode, configuredPct := copyGuardProtectionSettings(cycle, ti.engine.config); mode == store.RiskProtectionModePositionMarginPct {
			metadata := map[string]interface{}{
				"source": "exchange_hosted", "trigger_type": "mark", "algo_id": stored.AlgoID,
				"actual_order_id": live.ActualOrderID, "configured_margin_loss_pct": configuredPct,
				"trigger_price": stored.TriggerPrice, "fill_price": fillPrice,
				"quantity": stored.Quantity, "slippage": slippage, "fee": fee,
			}
			if attempts, attemptsErr := ti.store.CopyTrade().ListCopyGuardAttempts(cycle.ID); attemptsErr == nil {
				for _, attempt := range attempts {
					if attempt.AttemptNo != cycle.ReentryCount {
					...
					metadata["stop_anchor_entry_price"] = attempt.StopAnchorEntryPrice
					metadata["stop_anchor_leverage"] = attempt.StopAnchorLeverage
					metadata["stop_anchor_initial_margin"] = attempt.StopAnchorInitialMargin
					metadata["stop_anchor_price"] = attempt.StopAnchorPrice
					metadata["current_entry_price"] = attempt.EntryPrice
					metadata["current_quantity"] = attempt.Quantity
					metadata["current_leverage"] = attempt.ActualLeverage
					metadata["current_effective_margin_loss_pct"] = attempt.ExpectedPositionLossPct
					break
				}
			}
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
				CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_STOP_TRIGGERED",
				Price: fillPrice, Quantity: stored.Quantity, Notional: stored.Quantity * fillPrice,
				Fee: fee, PnL: stopPnL, Metadata: metadata,
			})
		}
...
func protectiveOrderCoverage(coverageMode string, protected, position float64) float64 {
	if coverageMode == trader.ProtectiveStopCoverageCloseAll {
		return 1
	}
	return protectionCoverage(protected, position)
}

func protectiveOrderQuantityMatches(coverageMode string, protected, position, quantityStep float64) bool {
	if coverageMode == trader.ProtectiveStopCoverageCloseAll {
		return true
	}
	return protectionQuantityMatches(protected, position, quantityStep)
}

// auditCopyGuardPositionOwnership compares the exchange position with the
// quantity reconstructed from durable, cycle-bound execution intents. It only
// classifies a mismatch when that ledger is complete and quiescent. Protection
// continues against the real exchange quantity; only evaluation eligibility is
// revoked because mixed ownership makes PnL attribution unknowable.
func (ti *TraderIntegration) auditCopyGuardPositionOwnership(cycle *store.CopyGuardCycle, actualQuantity, quantityStep float64) {
	if ti == nil || ti.store == nil || cycle == nil || actualQuantity < 0 {
		return
	}
	expected, err := ti.store.CopyTrade().GetCopyGuardPositionOwnershipExpectation(cycle.ID)
	if err != nil {
		logger.Warnf("⚠️ [%s] Copy Guard 仓位归属审计读取失败 | cycle=%d: %v", ti.traderID, cycle.ID, err)
		return
	}
	if !expected.Reliable || expected.InFlight {
		return
	}
	// A just-committed fill and the first protection refresh share a transaction
	// boundary but not a process mutex. This short grace avoids declaring an
	// anomaly while the foreground goroutine is about to refresh venue position
	// state; stable external mutations remain visible on later polls.
	if !expected.LastIntentUpdated.IsZero() && time.Since(expected.LastIntentUpdated) < 15*time.Second {
		return
	}
	tolerance := math.Max(quantityStep, math.Max(expected.ExpectedQuantity, actualQuantity)*1e-8)
	if math.Abs(expected.ExpectedQuantity-actualQuantity) <= tolerance+1e-12 {
		return
	}
	changed, markErr := ti.store.CopyTrade().MarkCopyGuardPositionOwnershipAnomaly(
		cycle.ID, ti.traderID, expected.ExpectedQuantity, actualQuantity, tolerance, expected.ContributingIntents,
	)
	if markErr != nil {
		logger.Errorf("❌ [%s] Copy Guard 仓位归属异常持久化失败 | cycle=%d: %v", ti.traderID, cycle.ID, markErr)
		return
	}
	if !changed {
		return
	}
	logger.Errorf("🚨 [%s] Copy Guard 检测到非本程序可解释的仓位变化 | cycle=%d %s %s expected=%.8f actual=%.8f",
		ti.traderID, cycle.ID, cycle.Symbol, cycle.Side, expected.ExpectedQuantity, actualQuantity)
	ti.notifyProtection(cycle, "Copy Guard 检测到执行账户仓位归属异常",
		fmt.Sprintf("%s %s 的实际仓位 %.8f 无法由本程序成交记录 %.8f 解释。跟单和全仓保护继续运行，但本周期不再进入策略评分；请勿手工或由其他程序操作该执行账户。", cycle.Symbol, cycle.Side, actualQuantity, expected.ExpectedQuantity),
		fmt.Sprintf("position_ownership_anomaly_%d", cycle.ID))
}
...
const (
	// protectionRearmWindow / protectionRearmWindowMax bound how many times one
	// cycle may re-arm its protective stop before the chain is treated as
	// oscillating rather than recovering. protection_retries deliberately resets on
	// a healthy status (so unrelated transient faults cannot accumulate into a
	// forced exit), which leaves "arm succeeds → verification fails → arm again"
	// running at poll cadence forever. Production spent 38 minutes at 3s intervals
	// this way, writing ~110k events.
	protectionRearmWindow    = 10 * time.Minute
	protectionRearmWindowMax = 12

	// protectionRearmThrottleDelay is the cadence adopted once oscillation is
	// detected. Retrying continues (the position may still be under-protected) but
	// slowly, and the operator is told once instead of every poll.
	protectionRearmThrottleDelay = 5 * time.Minute
)

// noteProtectionRearm records one re-arm for the cycle and reports whether the
// cycle is now in the throttled (slow) cadence. Alerting happens exactly once per
// window, on the transition into throttling.
func (ti *TraderIntegration) noteProtectionRearm(cycle *store.CopyGuardCycle) bool {
	if cycle == nil {
		return false
	}
	now := time.Now()
	tracker := ti.protectionRearms[cycle.ID]
	if tracker == nil || now.Sub(tracker.windowStart) >= protectionRearmWindow {
		ti.protectionRearms[cycle.ID] = &protectionRearmTracker{windowStart: now, count: 1}
		// Bound the map across long uptimes; closed cycles never come back.
		if len(ti.protectionRearms) > 512 {
			for id, t := range ti.protectionRearms {
				if now.Sub(t.windowStart) >= 2*protectionRearmWindow {
					delete(ti.protectionRearms, id)
				}
			}
		}
		return false
	...
	tracker.count++
	if tracker.count < protectionRearmWindowMax {
		return false
	}
	if !tracker.throttled {
		tracker.throttled = true
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "PROTECTION_RETRY_THROTTLED", Metadata: map[string]interface{}{"rearms": tracker.count, "window_minutes": protectionRearmWindow.Minutes(), "next_interval_minutes": protectionRearmThrottleDelay.Minutes(), "protection_status": cycle.ProtectionStatus, "last_error": cycle.ProtectionError}})
		logger.Warnf("⚠️ [%s] Copy Guard 保护单重挂震荡，转入慢速重试 | cycle=%d rearms=%d/%.0fmin status=%s err=%s",
			ti.traderID, cycle.ID, tracker.count, protectionRearmWindow.Minutes(), cycle.ProtectionStatus, cycle.ProtectionError)
		ti.notifyProtectionForEvent(cycle, "Copy Guard 保护单反复重挂",
			fmt.Sprintf("该仓位的止损保护在 %.0f 分钟内重挂了 %d 次仍未稳定验证，系统已转为每 %.0f 分钟重试一次以停止刷屏。\n保护状态尚未稳定验证，建议人工核对交易所仓位、止损单及其数量。\n当前状态: %s\n最近错误: %s",
				protectionRearmWindow.Minutes(), tracker.count, protectionRearmThrottleDelay.Minutes(), cycle.ProtectionStatus, cycle.ProtectionError),
			"rearm_throttled", "PROTECTION_RETRY_THROTTLED")
	}
	return true
}

// protectionRearmThrottled reports whether the cycle is currently in the slow
// cadence, without recording a new re-arm.
func (ti *TraderIntegration) protectionRearmThrottled(cycleID int64) bool {
	tracker := ti.protectionRearms[cycleID]
	if tracker == nil || time.Since(tracker.windowStart) >= protectionRearmWindow {
		return false
	}
	return tracker.throttled
}
...
func (ti *TraderIntegration) retryDegradedV4Protections() {
	...
	for _, cycle := range cycles {
		...
		case store.CopyGuardProtectionPending, store.CopyGuardProtectionUnknown, store.CopyGuardProtectionDegraded, store.CopyGuardProtectionUnprotectable, store.CopyGuardProtectionUnprotectedWarning, store.CopyGuardProtectionForcedExitPending, store.CopyGuardProtectionFlatReconciling:
		default:
			continue
		}
		...
		} else if ti.protectionRearmThrottled(cycle.ID) && retryDelay < protectionRearmThrottleDelay {
			// Oscillating cycle: keep retrying, but stop doing it at poll cadence.
			retryDelay = protectionRearmThrottleDelay
		}
		if cycle.ProtectionLastRetryAt != nil && time.Since(*cycle.ProtectionLastRetryAt) < retryDelay {
			continue
		}
		recordRetryEvent := ti.shouldRecordProtectionEvent(cycle.ID, "PROTECTION_RETRY", cycle.ProtectionStatus+"|"+cycle.ProtectionError)
		claimed, claimErr := ti.store.CopyTrade().BeginCopyGuardProtectionRetry(cycle, retryDelay, recordRetryEvent)
		if claimErr != nil {
		...
		if cycle.ProtectionStatus == store.CopyGuardProtectionForcedExitPending {
			remaining, known := ti.copyGuardFollowerQuantity(cycle, true)
...
// handlePositionMarginStopCrossed handles the narrow race where the immutable
// trigger (or a liquidation-safety tightening) is already crossed before the
// venue can arm/amend it. This is a real policy stop, not an "unprotectable"
// configuration failure: exit is idempotent, then the normal stopped_by_risk
// lifecycle suppresses all signals until the leader fully closes.
func (ti *TraderIntegration) handlePositionMarginStopCrossed(dec *decision.Decision, cycle *store.CopyGuardCycle, side string, quantity, currentPrice float64, evaluation *PositionMarginStopEvaluation) {
	if !ti.copyGuardOwnsPosition(cycle) {
		return
	}
	if dec == nil || cycle == nil || evaluation == nil {
		return
	}
	// Install the submission gate before any best-effort audit or mapping read.
	_ = ti.establishRiskExitGate(store.CopyGuardRiskExitBegin{
		CycleID: cycle.ID, TraderID: ti.traderID, LeaderPosID: cycle.LeaderPosID,
		AttemptNo: cycle.ReentryCount, TriggerPrice: evaluation.StopPrice, Quantity: quantity,
		TriggerSource: "verified_mark_crossing", Metadata: map[string]interface{}{"mark_price": currentPrice},
	})
	reason := fmt.Sprintf("fixed position-margin stop %.8f crossed by mark %.8f", evaluation.StopPrice, currentPrice)
	key := fmt.Sprintf("%d|%d", cycle.ReentryCount, protectionSignatureBucket(evaluation.StopPrice))
	if ti.shouldRecordProtectionEvent(cycle.ID, "POSITION_MARGIN_STOP_TRIGGERED", key) {
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
			CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_STOP_TRIGGERED",
			Price: currentPrice, Quantity: quantity, Notional: currentPrice * quantity,
			Metadata: map[string]interface{}{
				"attempt_no": cycle.ReentryCount, "stop_price": evaluation.StopPrice,
				"current_effective_margin_loss_pct": evaluation.CurrentEffectiveMarginLossPct,
				"current_risk_usd":                  evaluation.CurrentRiskUSD, "clamped": evaluation.Clamped,
				"trigger_type": "mark", "reason": reason,
			},
		})
	}
	leaderPnL, leaderSize, addCount := float64(0), float64(0), 0
	if ti.engine != nil {
		if leader := ti.engine.buildLeaderPosMap()[cycle.LeaderPosID]; leader != nil {
			leaderPnL, leaderSize = leader.UnrealizedPnL, leader.Size
		}
	}
	if mapping, mappingErr := ti.store.CopyTrade().GetMapping(ti.traderID, cycle.LeaderPosID); mappingErr == nil && mapping != nil {
		addCount = mapping.AddCount
	}
	begin := store.CopyGuardRiskExitBegin{
		CycleID: cycle.ID, TraderID: ti.traderID, LeaderPosID: cycle.LeaderPosID,
		AttemptNo: cycle.ReentryCount, TriggerPrice: evaluation.StopPrice, Quantity: quantity,
		TriggerSource: "verified_mark_crossing", LeaderPnL: leaderPnL, LeaderSize: leaderSize,
		AddCount: addCount, Metadata: map[string]interface{}{"mark_price": currentPrice, "reason": reason},
	}
	if err := ti.establishRiskExitGate(begin); err != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_BEGIN_FAILED", err, cycle.ProtectionCoverage, true)
		ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "POSITION_MARGIN_STOP_BEGIN_PENDING", reason)
		return
	}
	_, ok := ti.executor.(EmergencyPositionCloser)
	if !ok {
		err := fmt.Errorf("executor cannot market-close an already-crossed fixed stop")
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_EXIT_FAILED", err, cycle.ProtectionCoverage, true)
		return
	}
	orderID, err := ti.closeCopyGuardPosition(cycle)
	if err != nil && !ti.isBenignCloseError(&decision.Decision{Action: "close_" + side, Symbol: cycle.Symbol, MarginMode: cycle.MarginMode}, err) {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_EXIT_FAILED", err, cycle.ProtectionCoverage, true)
		return
	}
	if orderID != "" {
		_ = ti.store.CopyTrade().UpdateCopyGuardAttemptIdentity(cycle.ID, cycle.ReentryCount, "", "", orderID)
	}
	_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_STOP_EXIT_SUBMITTED", Price: currentPrice, Quantity: quantity, Metadata: map[string]interface{}{"order_id": orderID, "stop_price": evaluation.StopPrice, "reason": reason}})
	remaining, known := ti.copyGuardFollowerQuantity(cycle, true)
	if !known || remaining > 1e-12 {
		ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "POSITION_MARGIN_STOP_PENDING_FLAT", reason)
		return
	}
	if !ti.finalizePositionMarginStopExit(cycle, currentPrice) {
		ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "POSITION_MARGIN_STOP_FINALIZE_PENDING", reason)
		return
	}
	ti.transitionExecutionIntent(dec, store.ExecutionIntentFilled, "POSITION_MARGIN_STOP_FLAT_CONFIRMED", reason)
}

// reconcilePositionMarginStopPendingExits closes the acknowledgement gap of a
// reduce-only market exit. A submission is not proof of a fill, and the process
// may restart while the exchange is still settling it. Fixed-stop cycles remain
// durably pending until a fresh position read proves flat and stale protection
// has been canceled; residual quantity is retried at a bounded cadence.
func (ti *TraderIntegration) reconcilePositionMarginStopPendingExits() {
	ti.protectionMu.Lock()
	defer ti.protectionMu.Unlock()
	...
	for _, cycle := range cycles {
		mode, _ := copyGuardProtectionSettings(cycle, ti.engine.config)
		if (mode != store.RiskProtectionModePositionMarginPct && !copyGuardLifecycleConfig(cycle, ti.engine.config).RiskLiquidationGuardEnabled) ||
			(cycle.Status != store.CopyGuardStopPendingFlat && cycle.Status != store.CopyGuardStopPartial) {
			continue
		}
		remaining, known := ti.copyGuardFollowerQuantity(cycle, true)
		if !known {
			if ti.shouldRecordProtectionEvent(cycle.ID, "POSITION_MARGIN_STOP_FLAT_UNKNOWN", cycle.FollowerPosID) {
				ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_FLAT_UNKNOWN", fmt.Errorf("fresh follower position cannot be attributed"), cycle.ProtectionCoverage, false)
			}
			continue
		}
		markPrice, _ := ti.fixedStopReferencePrice(cycle.Symbol, 0)
		if remaining <= 1e-12 {
			if ti.finalizePositionMarginStopExit(cycle, markPrice) {
				delete(ti.residualCloseLast, cycle.ID)
				delete(ti.residualCloseTries, cycle.ID)
			}
			continue
		}
		_, _ = ti.store.CopyTrade().MarkCopyGuardRiskExitPartial(cycle.ID, cycle.ReentryCount, remaining, markPrice,
			map[string]interface{}{"trigger_source": "verified_mark_crossing", "phase": "residual_reconcile"})
		if last := ti.residualCloseLast[cycle.ID]; !last.IsZero() && time.Since(last) < 10*time.Second {
			continue
		}
		ti.residualCloseLast[cycle.ID] = time.Now()
		ti.residualCloseTries[cycle.ID]++
		_, ok := ti.executor.(EmergencyPositionCloser)
		if !ok {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_EXIT_FAILED", fmt.Errorf("executor cannot market-close fixed-stop residual"), cycle.ProtectionCoverage, false)
			continue
		}
		orderID, closeErr := ti.closeCopyGuardPosition(cycle)
		if closeErr != nil && !ti.isBenignCloseError(&decision.Decision{Action: "close_" + cycle.Side, Symbol: cycle.Symbol, MarginMode: cycle.MarginMode}, closeErr) {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_EXIT_FAILED", closeErr, cycle.ProtectionCoverage, true)
			continue
		}
		if orderID != "" {
			_ = ti.store.CopyTrade().UpdateCopyGuardAttemptIdentity(cycle.ID, cycle.ReentryCount, "", "", orderID)
		...
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
			CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_STOP_RESIDUAL_EXIT_SUBMITTED",
			Price: markPrice, Quantity: remaining, Notional: markPrice * remaining,
			Metadata: map[string]interface{}{"order_id": orderID, "attempt": ti.residualCloseTries[cycle.ID], "remaining_quantity": remaining},
		})
	}
}

// positionMarginAnchorInitializationAllowed is the crash/retry boundary for a
// missing immutable anchor. A later add, reduce or catch-up must never promote
// the then-current weighted average into a counterfeit first fill. Recovered
// initial executions have no CopyTradeAction, so the durable mapping counters
// are the secondary proof that no position transition has happened yet.
func positionMarginAnchorInitializationAllowed(dec *decision.Decision, mapping *store.CopyTradePositionMapping, reentryCount int) bool {
	if dec == nil || mapping == nil || dec.ExecutionIntentID <= 0 || reentryCount != 0 ||
		(dec.Action != "open_long" && dec.Action != "open_short") ||
		mapping.Status != store.MappingStatusActive || mapping.AddCount != 0 || mapping.ReduceCount != 0 {
		return false
	}
	action := strings.ToLower(strings.TrimSpace(dec.CopyTradeAction))
	switch action {
	case "open":
		return true
	case "add", "reduce", "catchup", "ai_reentry", "protection_reprice":
		return false
	case "":
		return !strings.Contains(strings.ToLower(dec.Reasoning), "add") &&
			!strings.Contains(strings.ToLower(dec.Reasoning), "reduce") &&
...
func (ti *TraderIntegration) checkpointPositionMarginShadowMark(cycleID int64, effectiveStop, markPrice float64, force bool) (bool, error) {
	if ti == nil || ti.store == nil || cycleID <= 0 || effectiveStop <= 0 || markPrice <= 0 {
		return false, nil
	}
	now := time.Now().UTC()
	ti.shadowMarkMu.Lock()
	defer ti.shadowMarkMu.Unlock()
	if ti.shadowMarks == nil {
		ti.shadowMarks = make(map[int64]*shadowMarkAccumulator)
	}
	acc := ti.shadowMarks[cycleID]
	if acc == nil {
		acc = &shadowMarkAccumulator{bucketAt: now.Truncate(time.Minute), firstObservedAt: now}
		ti.shadowMarks[cycleID] = acc
	}
	if !acc.lastObservedAt.IsZero() {
		delta := now.Sub(acc.lastObservedAt).Seconds()
		if delta > 0 {
			if delta <= 15 {
				acc.coveredSeconds += delta
			} else {
				acc.gapSeconds += delta
			}
		}
	}
	acc.lastObservedAt = now
	if acc.observationCount > 0 {
		acc.minimumMark = math.Min(acc.minimumMark, markPrice)
		acc.maximumMark = math.Max(acc.maximumMark, markPrice)
	} else {
		acc.minimumMark, acc.maximumMark = markPrice, markPrice
	}
	acc.lastMark = markPrice
	acc.observationCount++
	if !force && now.Sub(acc.firstObservedAt) < time.Minute {
		return false, nil
	}
	return ti.persistPositionMarginShadowAccumulatorLocked(cycleID, effectiveStop, acc)
}

// persistPositionMarginShadowAccumulatorLocked writes one minute/event bucket.
// The caller holds shadowMarkMu across the database commit so a concurrent
// lifecycle close cannot discard observations appended during the flush.
func (ti *TraderIntegration) persistPositionMarginShadowAccumulatorLocked(cycleID int64, effectiveStop float64, acc *shadowMarkAccumulator) (bool, error) {
	if acc == nil || acc.observationCount <= 0 {
		return false, nil
	}
	cp := store.CopyGuardShadowMarkCheckpoint{
		Key:      fmt.Sprintf("live:%d:%d:%d", acc.firstObservedAt.UnixNano(), acc.lastObservedAt.UnixNano(), acc.observationCount),
		BucketAt: acc.bucketAt, MinimumMark: acc.minimumMark, MaximumMark: acc.maximumMark,
		LastMark: acc.lastMark, ObservationCount: acc.observationCount,
		CoveredSeconds: acc.coveredSeconds, GapSeconds: acc.gapSeconds,
		ObservedAt: acc.lastObservedAt, Source: "LIVE_MARK",
	}
	crossed, err := ti.store.CopyTrade().CheckpointCopyGuardPositionMarginShadowV2(cycleID, cp, effectiveStop)
	if err == nil && ti.shadowMarks[cycleID] == acc {
		// Preserve the last covered instant across minute/event buckets. Dropping
		// the accumulator here made the interval to the next live sample vanish
		// from both covered_seconds and gap_seconds, which could overstate mark
		// coverage after every flush.
		ti.shadowMarks[cycleID] = &shadowMarkAccumulator{
			bucketAt:        acc.lastObservedAt.Truncate(time.Minute),
			firstObservedAt: acc.lastObservedAt,
...
func (ti *TraderIntegration) flushPositionMarginShadowMarks(cycleID int64, effectiveStop float64) error {
	if ti == nil || ti.store == nil || cycleID <= 0 || effectiveStop <= 0 {
		return nil
	}
	ti.shadowMarkMu.Lock()
	defer ti.shadowMarkMu.Unlock()
	_, err := ti.persistPositionMarginShadowAccumulatorLocked(cycleID, effectiveStop, ti.shadowMarks[cycleID])
	return err
}

// finalizeCopyGuardPositionMarginShadow is the lifecycle-event checkpoint. It
// persists the incomplete current minute before the v2 ledger is frozen, then
// delegates to the existing idempotent v1/v2 finalizer.
func (ti *TraderIntegration) finalizeCopyGuardPositionMarginShadow(cycleID int64, closePrice float64) error {
	...
	if shadow, err := ti.store.CopyTrade().GetCopyGuardPositionMarginShadowV2(cycleID); err == nil {
		if flushErr := ti.flushPositionMarginShadowMarks(cycleID, shadow.EffectiveStopPrice); flushErr != nil {
			return flushErr
		}
	} else if !errors.Is(err, sql.ErrNoRows) {
		return err
	}
	err := ti.store.CopyTrade().FinalizeCopyGuardPositionMarginShadow(cycleID, closePrice)
	if err == nil {
		ti.shadowMarkMu.Lock()
		delete(ti.shadowMarks, cycleID)
		ti.shadowMarkMu.Unlock()
	}
	return err
}

func (ti *TraderIntegration) seedPositionMarginShadowMarkClock(cycleID int64, previous time.Time) {
	if previous.IsZero() {
		return
	}
	now := time.Now().UTC()
	...
	if ti.shadowMarks[cycleID] == nil {
		ti.shadowMarks[cycleID] = &shadowMarkAccumulator{
			bucketAt: now.Truncate(time.Minute), firstObservedAt: now,
			lastObservedAt: previous.UTC(),
		}
	}
}

func (ti *TraderIntegration) resetPositionMarginShadowMarkClock(cycleID int64, previous time.Time) {
	if previous.IsZero() {
		return
	}
	now := time.Now().UTC()
	...
	// Completed history is authoritative for the overlap accumulated since a
	// restart. Discard that in-memory overlap and continue from the last repaired
	// candle so coverage seconds cannot be double-counted.
	ti.shadowMarks[cycleID] = &shadowMarkAccumulator{
		bucketAt: now.Truncate(time.Minute), firstObservedAt: now,
		lastObservedAt: previous.UTC(),
	}
...
// backfillPositionMarginShadowMarks repairs restart/disconnect gaps with the
// execution venue's completed one-minute mark candles. It returns the latest
// covered instant so the next live sample accounts only for any still-missing
// tail. Failed or incomplete history remains an explicit gap and therefore
// cannot satisfy the 95% promotion gate.
func (ti *TraderIntegration) backfillPositionMarginShadowMarks(cycle *store.CopyGuardCycle, shadow *store.CopyGuardPositionMarginShadowV2, effectiveStop float64) (bool, time.Time) {
	if cycle == nil || shadow == nil {
		return false, time.Time{}
	}
	resumeAt := cycle.OpenedAt.UTC()
	if shadow.LastMarkAt != nil {
		resumeAt = shadow.LastMarkAt.UTC()
	}
	if resumeAt.IsZero() {
		return false, time.Time{}
	}
	now := time.Now().UTC()
	if now.Sub(resumeAt) <= 75*time.Second {
		return false, resumeAt
	}
	provider, ok := ti.executor.(trader.MarkPriceHistoryProvider)
	if !ok {
		return false, resumeAt
	}
	candles, err := provider.GetMarkPriceHistory(cycle.Symbol, resumeAt, now)
	if err != nil {
		logger.Warnf("⚠️ [%s] 80%% 影子 mark 历史补洞失败 | cycle=%d: %v", ti.traderID, cycle.ID, err)
		return false, resumeAt
	}
	crossedAny := false
	cursor := resumeAt
	for _, candle := range candles {
		start, end := candle.OpenTime.UTC(), candle.CloseTime.UTC()
		if start.Before(resumeAt) {
			start = resumeAt
		}
		if end.After(now) {
			end = now
		}
		if !start.Before(end) || candle.Low <= 0 || candle.High <= 0 || candle.Close <= 0 {
			continue
		}
		gap := float64(0)
		if start.After(cursor) {
			gap = start.Sub(cursor).Seconds()
		}
		cp := store.CopyGuardShadowMarkCheckpoint{
			Key:      fmt.Sprintf("history:%d", candle.OpenTime.UTC().UnixMilli()),
			BucketAt: candle.OpenTime.UTC().Truncate(time.Minute), MinimumMark: candle.Low,
			MaximumMark: candle.High, LastMark: candle.Close, ObservationCount: 1,
			CoveredSeconds: end.Sub(start).Seconds(), GapSeconds: gap,
			ObservedAt: end, Source: "HISTORY_MARK_1M",
		}
		crossed, checkpointErr := ti.store.CopyTrade().CheckpointCopyGuardPositionMarginShadowV2(cycle.ID, cp, effectiveStop)
		if checkpointErr != nil {
			logger.Warnf("⚠️ [%s] 80%% 影子 mark 补洞落库失败 | cycle=%d: %v", ti.traderID, cycle.ID, checkpointErr)
			return crossedAny, cursor
		}
		crossedAny = crossedAny || crossed
		if end.After(cursor) {
			cursor = end
		}
	}
...
func (ti *TraderIntegration) syncPositionMarginStopShadow(dec *decision.Decision, cycle *store.CopyGuardCycle, side SideType, entryPrice, quantity float64, leverage int, tickSize, quantityStep, positionMarkPrice float64) {
	if !store.CopyGuardShadowRuntimeEnabled {
		return
	}
	if dec == nil || cycle == nil || side == "" || entryPrice <= 0 || quantity <= 0 || leverage <= 0 || tickSize <= 0 || quantityStep <= 0 {
		return
	}
	existing, existingErr := ti.store.CopyTrade().GetCopyGuardPositionMarginShadow(cycle.ID)
	if existingErr != nil && !errors.Is(existingErr, sql.ErrNoRows) {
		logger.Warnf("⚠️ [%s] 读取 80%% 影子止损失败 | cycle=%d: %v", ti.traderID, cycle.ID, existingErr)
		return
	}
	if errors.Is(existingErr, sql.ErrNoRows) {
		mapping, mappingErr := ti.store.CopyTrade().GetMapping(ti.traderID, cycle.LeaderPosID)
		if mappingErr != nil || cycle.InitialIntentID <= 0 || dec.ExecutionIntentID != cycle.InitialIntentID ||
			!positionMarginAnchorInitializationAllowed(dec, mapping, cycle.ReentryCount) {
			// An old/recovered ATR lifecycle has no trustworthy first follower
			// fill. Starting from a later weighted average would manufacture a
			// false "first-entry" backtest, so leave it explicitly unscored.
			return
		}
		plan, planErr := BuildPositionMarginStopAnchor(side, entryPrice, quantity, float64(leverage), store.DefaultRiskPositionMarginStopPct, tickSize)
		if planErr != nil {
			logger.Warnf("⚠️ [%s] 生成 80%% 影子止损失败 | cycle=%d: %v", ti.traderID, cycle.ID, planErr)
			return
		}
		createdShadow, created, createErr := ti.store.CopyTrade().InitializeCopyGuardPositionMarginShadow(&store.CopyGuardPositionMarginShadow{
			CycleID: cycle.ID, TraderID: ti.traderID, Side: string(side),
			AnchorEntryPrice: plan.EntryPrice, AnchorLeverage: plan.Leverage,
			AnchorInitialMargin: plan.InitialMargin, AnchorStopPrice: plan.StopPrice,
			ConfiguredMarginLossPct: plan.ConfiguredMarginLossPct,
			PriceTickSize:           tickSize, QuantityStep: quantityStep,
			InitialQuantity: quantity, CurrentEntryPrice: entryPrice,
			CurrentQuantity: quantity, CurrentLeverage: float64(leverage),
			EffectiveStopPrice: plan.StopPrice, LastLeaderSize: dec.LeaderPosSize,
		})
		if createErr != nil {
			logger.Warnf("⚠️ [%s] 固化 80%% 影子止损失败 | cycle=%d: %v", ti.traderID, cycle.ID, createErr)
			return
		}
		existing = createdShadow
		if created {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
				CycleID: cycle.ID, TraderID: ti.traderID, Type: "SHADOW_POSITION_MARGIN_ANCHOR_FROZEN",
				Price: plan.StopPrice, Quantity: quantity, Notional: entryPrice * quantity,
				Metadata: map[string]interface{}{
					"policy":             store.CopyGuardShadowFirstEntryPositionMargin80,
					"anchor_entry_price": plan.EntryPrice, "anchor_leverage": plan.Leverage,
					"anchor_initial_margin": plan.InitialMargin, "anchor_stop_price": plan.StopPrice,
					"configured_margin_loss_pct": plan.ConfiguredMarginLossPct,
					"orders_placed":              false, "fees_funding_slippage_excluded": true,
				},
			})
		}
	}
	if existing == nil || existing.Status != store.CopyGuardPositionMarginShadowActive {
		return
	}
	if _, v2Err := ti.store.CopyTrade().GetCopyGuardPositionMarginShadowV2(cycle.ID); errors.Is(v2Err, sql.ErrNoRows) {
		mapping, mappingErr := ti.store.CopyTrade().GetMapping(ti.traderID, cycle.LeaderPosID)
		if mappingErr == nil && cycle.InitialIntentID > 0 && dec.ExecutionIntentID == cycle.InitialIntentID &&
			positionMarginAnchorInitializationAllowed(dec, mapping, cycle.ReentryCount) {
			costBPS := float64(0)
			if policy, policyErr := store.DecodeCopyGuardPolicySnapshot(cycle.PolicySnapshot); policyErr == nil {
				costBPS = math.Max(0, policy.RoundTripFeeBPS) + math.Max(0, policy.SlippageBufferBPS)
			}
			_, _, v2Err = ti.store.CopyTrade().InitializeCopyGuardPositionMarginShadowV2(&store.CopyGuardPositionMarginShadowV2{
				CycleID: cycle.ID, TraderID: ti.traderID, Side: string(side),
				AnchorEntryPrice: existing.AnchorEntryPrice, AnchorLeverage: existing.AnchorLeverage,
				AnchorInitialMargin: existing.AnchorInitialMargin, AnchorStopPrice: existing.AnchorStopPrice,
				ConfiguredMarginLossPct: existing.ConfiguredMarginLossPct,
				PriceTickSize:           tickSize, QuantityStep: quantityStep, InitialQuantity: existing.InitialQuantity,
				CurrentEntryPrice: entryPrice, CurrentQuantity: quantity, CurrentLeverage: float64(leverage),
				EffectiveStopPrice: existing.EffectiveStopPrice, LastLeaderSize: dec.LeaderPosSize,
				ConfiguredCostBPS: costBPS,
			})
			if v2Err != nil {
				logger.Warnf("⚠️ [%s] 初始化 80%% 影子账本 v2 失败 | cycle=%d: %v", ti.traderID, cycle.ID, v2Err)
			}
		}
	} else if v2Err != nil {
		logger.Warnf("⚠️ [%s] 读取 80%% 影子账本 v2 失败 | cycle=%d: %v", ti.traderID, cycle.ID, v2Err)
	}
	changed, updateErr := ti.store.CopyTrade().UpdateCopyGuardPositionMarginShadowPosition(cycle.ID, entryPrice, quantity, float64(leverage), dec.LeaderPosSize)
	if updateErr != nil {
		logger.Warnf("⚠️ [%s] 更新 80%% 影子仓位失败 | cycle=%d: %v", ti.traderID, cycle.ID, updateErr)
		return
	}
	v2Changed, v2Err := ti.store.CopyTrade().SyncCopyGuardPositionMarginShadowV2(cycle.ID, entryPrice, quantity, float64(leverage), dec.LeaderPosSize, dec.EntryPrice)
	if v2Err != nil {
		logger.Warnf("⚠️ [%s] 更新 80%% 影子账本 v2 失败 | cycle=%d: %v", ti.traderID, cycle.ID, v2Err)
	}
	if changed || v2Changed {
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
			CycleID: cycle.ID, TraderID: ti.traderID, Type: "SHADOW_POSITION_MARGIN_QUANTITY_SYNCED",
			Price: existing.EffectiveStopPrice, Quantity: quantity, Notional: entryPrice * quantity,
			Metadata: map[string]interface{}{"current_entry_price": entryPrice, "current_leverage": leverage, "leader_size": dec.LeaderPosSize, "orders_placed": false},
		})
	}
	// Initial fills and every confirmed add/reduce are lifecycle checkpoints.
	// Persist the execution venue's authoritative mark immediately so a crash or
	// leader close inside the current minute cannot leave an optimistic gap.
	if markPrice, _ := ti.stopTriggerReferencePriceForType(dec.Symbol, positionMarkPrice, "mark"); markPrice > 0 {
		if _, checkpointErr := ti.checkpointPositionMarginShadowMark(cycle.ID, existing.EffectiveStopPrice, markPrice, true); checkpointErr != nil {
			logger.Warnf("⚠️ [%s] 保存 80%% 影子生命周期 mark 检查点失败 | cycle=%d: %v", ti.traderID, cycle.ID, checkpointErr)
		}
	}
}

// observePositionMarginStopShadows reads prices on the normal three-second
// protection cadence but writes only on a one-way clamp or the first crossing.
// It is a passive evaluator and never submits, amends or cancels an order.
func (ti *TraderIntegration) observePositionMarginStopShadows() {
	if !store.CopyGuardShadowRuntimeEnabled {
		return
	}
	shadows, err := ti.store.CopyTrade().ListActiveCopyGuardPositionMarginShadows(ti.traderID)
	if err != nil || len(shadows) == 0 {
		return
	}
	// GetPositions may be backed by the normal 15-second account snapshot.
	// Reusing that value on this three-second evaluator cadence would count the
	// same cached mark as five independent observations and can miss an
	// intra-cache stop crossing. Query the venue's dedicated mark endpoint once
	// per symbol for this observation pass, falling back only to the
	// exchange-provided position mark when the endpoint is unavailable. Last
	// trade price is never a valid substitute for a mark-triggered stop.
	dedicatedMarkFetched := make(map[string]bool)
	dedicatedMarks := make(map[string]float64)
	leaderPositions := map[string]*Position{}
	if ti.engine != nil {
		leaderPositions = ti.engine.buildLeaderPosMap()
	}
	positions, positionsErr := ti.executor.GetPositions()
	for _, shadow := range shadows {
		cycle, cycleErr := ti.store.CopyTrade().GetCopyGuardCycle(shadow.CycleID)
		if cycleErr != nil {
			continue
		}
		if cycle.ClosedAt != nil {
			// Closing paths normally finalize before closing the cycle. If that
			// write failed or the process stopped in the middle, keep retrying the
			// idempotent upsert instead of leaving an ACTIVE shadow forever.
			_ = ti.finalizeCopyGuardPositionMarginShadow(cycle.ID, cycle.LastObservedPrice)
			continue
		}
		entryPrice, quantity, leverage := shadow.CurrentEntryPrice, shadow.CurrentQuantity, shadow.CurrentLeverage
		markPrice, liquidationPrice := float64(0), float64(0)
		priceSource := ""
		followerPositionFound := false
		if positionsErr == nil {
			for _, pos := range positions {
				...
				followerPositionFound = true
				if v := getFloatField(pos, "entryPrice", "entry_price"); v > 0 {
					entryPrice = v
				}
				if v := absFloat(getFloatField(pos, "positionAmt", "quantity")); v > 0 {
					quantity = v
				}
				if v := getIntOrFloatField(pos, "leverage"); v > 0 {
					leverage = float64(v)
				}
				markPrice = getFloatField(pos, "markPrice", "mark_price")
				liquidationPrice = getFloatField(pos, "liquidationPrice", "liquidation_price")
				priceSource = "follower_mark"
				break
			}
		}
		markKey := strings.ToUpper(strings.TrimSpace(cycle.Symbol))
		if !dedicatedMarkFetched[markKey] {
			dedicatedMarkFetched[markKey] = true
			dedicatedMarks[markKey], _ = ti.stopTriggerReferencePriceForType(cycle.Symbol, 0, "mark")
		}
		if dedicatedMark := dedicatedMarks[markKey]; dedicatedMark > 0 {
			markPrice = dedicatedMark
			priceSource = "mark_endpoint"
		} else if markPrice > 0 {
			priceSource = "follower_mark"
		}
		// Once the real ATR path has stopped, there is deliberately no follower
		// position to read. Keep the passive fixed-80% comparison aligned with
		// later leader adds/reductions even when reentry is disabled. A healthy
		// empty follower snapshot and an execution-venue mark are both required:
		// leader marks (and especially leader entry prices) are not valid virtual
		// fill prices on the follower exchange.
		if positionsErr == nil && !followerPositionFound && cycle.StoppedAt != nil {
			leader := leaderPositions[cycle.LeaderPosID]
			if leader != nil && leader.Size > 0 && strings.EqualFold(string(leader.Side), cycle.Side) {
				if markPrice > 0 {
					changed, shadowErr := ti.store.CopyTrade().AdvanceCopyGuardPositionMarginShadowByLeader(cycle.ID, leader.Size, markPrice)
					if shadowErr != nil {
						logger.Warnf("⚠️ [%s] 80%% 影子仓位同步失败: %v | cycle=%d", ti.traderID, shadowErr, cycle.ID)
					}
					v2Changed, v2Err := ti.store.CopyTrade().AdvanceCopyGuardPositionMarginShadowV2ByLeader(cycle.ID, leader.Size, markPrice, shadow.CurrentLeverage)
					if v2Err != nil {
						logger.Warnf("⚠️ [%s] 80%% 影子仓位 v2 同步失败: %v | cycle=%d", ti.traderID, v2Err, cycle.ID)
					}
					if changed || v2Changed {
						_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "SHADOW_POSITION_MARGIN_QUANTITY_SYNCED", Price: markPrice, Metadata: map[string]interface{}{"leader_size": leader.Size, "phase": "after_real_stop", "price_source": priceSource, "orders_placed": false}})
						if refreshed, refreshErr := ti.store.CopyTrade().GetCopyGuardPositionMarginShadow(cycle.ID); refreshErr == nil {
							shadow = refreshed
							entryPrice, quantity, leverage = shadow.CurrentEntryPrice, shadow.CurrentQuantity, shadow.CurrentLeverage
						}
					}
				} else if math.Abs(leader.Size-shadow.LastLeaderSize) > 1e-12 {
					reason := "execution mark unavailable for post-stop leader position transition"
					if markErr := ti.store.CopyTrade().MarkCopyGuardPositionMarginShadowV2Unscorable(cycle.ID, reason); markErr != nil {
						logger.Warnf("⚠️ [%s] 标记 80%% 影子账本不可评分失败 | cycle=%d: %v", ti.traderID, cycle.ID, markErr)
					} else {
						logger.Warnf("⚠️ [%s] %s | cycle=%d", ti.traderID, reason, cycle.ID)
					}
				}
			}
		}
		if entryPrice <= 0 || quantity <= 0 || leverage <= 0 || markPrice <= 0 {
			continue
		}
		evaluation, evalErr := EvaluatePositionMarginStop(PositionMarginStopEvaluationInput{
			Side: SideType(shadow.Side), CurrentEntryPrice: entryPrice,
			CurrentQuantity: quantity, CurrentLeverage: leverage,
			AnchorStopPrice: shadow.AnchorStopPrice, ExistingStopPrice: shadow.EffectiveStopPrice,
			MarkPrice: markPrice, LiquidationPrice: liquidationPrice,
			PriceTickSize: shadow.PriceTickSize, BaseQuantityStep: shadow.QuantityStep,
			ATRValue: cycle.ATRAtEntry, LiquidationBufferATR: copyGuardLiquidationBufferATR(cycle, ti.engine.config),
		})
		if evalErr != nil {
			continue
		}
		historyCrossed := false
		if shadowV2, shadowV2Err := ti.store.CopyTrade().GetCopyGuardPositionMarginShadowV2(cycle.ID); shadowV2Err == nil {
			previousPersisted := cycle.OpenedAt.UTC()
			if shadowV2.LastMarkAt != nil {
				previousPersisted = shadowV2.LastMarkAt.UTC()
			}
			var resumeAt time.Time
			historyCrossed, resumeAt = ti.backfillPositionMarginShadowMarks(cycle, shadowV2, evaluation.StopPrice)
			if resumeAt.IsZero() && shadowV2.LastMarkAt != nil {
				resumeAt = *shadowV2.LastMarkAt
			}
			if !previousPersisted.IsZero() && resumeAt.After(previousPersisted) {
				ti.resetPositionMarginShadowMarkClock(cycle.ID, resumeAt)
			} else {
				ti.seedPositionMarginShadowMarkClock(cycle.ID, resumeAt)
			}
		}
		crossed, tightened, observeErr := ti.store.CopyTrade().ObserveCopyGuardPositionMarginShadow(
			cycle.ID, evaluation.StopPrice, markPrice, entryPrice, quantity, leverage)
		if observeErr != nil {
			logger.Warnf("⚠️ [%s] 观察 80%% 影子止损失败 | cycle=%d: %v", ti.traderID, cycle.ID, observeErr)
			continue
		}
		v2Crossed, checkpointErr := ti.checkpointPositionMarginShadowMark(cycle.ID, evaluation.StopPrice, markPrice, tightened || crossed || historyCrossed)
		if checkpointErr != nil {
			logger.Warnf("⚠️ [%s] 保存 80%% 影子 mark 检查点失败 | cycle=%d: %v", ti.traderID, cycle.ID, checkpointErr)
		}
		if tightened {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "SHADOW_POSITION_MARGIN_LIQUIDATION_TIGHTENED", Price: evaluation.StopPrice, Quantity: quantity, Metadata: map[string]interface{}{"anchor_stop_price": shadow.AnchorStopPrice, "liquidation_price": liquidationPrice, "orders_placed": false}})
		}
		if crossed || v2Crossed || historyCrossed {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
				CycleID: cycle.ID, TraderID: ti.traderID, Type: "SHADOW_POSITION_MARGIN_STOP_CROSSED",
				Price: markPrice, Quantity: quantity, Notional: markPrice * quantity,
				Metadata: map[string]interface{}{
					"policy":               store.CopyGuardShadowFirstEntryPositionMargin80,
					"effective_stop_price": evaluation.StopPrice, "price_source": priceSource,
					"history_crossing":    historyCrossed,
					"current_entry_price": entryPrice, "current_leverage": leverage,
					"current_effective_margin_loss_pct": evaluation.CurrentEffectiveMarginLossPct,
					"orders_placed":                     false, "fees_funding_slippage_excluded": true,
				},
			})
...
func (ti *TraderIntegration) finalizePositionMarginStopExit(cycle *store.CopyGuardCycle, observedPrice float64) bool {
	if cycle == nil {
		return false
	}
	remaining, known := ti.copyGuardFollowerQuantity(cycle, true)
	...
	if order, orderErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID); orderErr == nil {
		mgr, managerOK := ti.executor.(ProtectiveStopManagerV4)
		if !managerOK {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "PROTECTION_CANCEL_UNCONFIRMED", fmt.Errorf("executor cannot cancel stale protective order"), cycle.ProtectionCoverage, false)
			return false
		}
		...
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "PROTECTION_QUERY_UNCONFIRMED", orderErr, cycle.ProtectionCoverage, false)
		return false
	}
	attempts, attemptErr := ti.store.CopyTrade().ListCopyGuardAttempts(cycle.ID)
	if attemptErr != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_ATTEMPT_UNAVAILABLE", attemptErr, 0, false)
		return false
	}
	var attempt *store.CopyGuardAttempt
	for _, candidate := range attempts {
		if candidate.AttemptNo == cycle.ReentryCount {
			attempt = candidate
			break
		}
	}
	if attempt == nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_ATTEMPT_UNAVAILABLE", fmt.Errorf("attempt %d not found", cycle.ReentryCount), 0, false)
		return false
	}
	stopPrice := attempt.StopTriggerPrice
	if stopPrice <= 0 {
	...
	fillPrice, fee := observedPrice, float64(0)
	if attempt.ExitOrderID != "" {
		if provider, ok := ti.executor.(OrderStatusProvider); ok {
			if status, statusErr := provider.GetOrderStatus(cycle.Symbol, attempt.ExitOrderID); statusErr == nil {
				if price := getFloatField(status, "avgPrice"); price > 0 {
					fillPrice = price
				}
				fee = getFloatField(status, "commission")
			}
	...
	if fillPrice <= 0 {
		fillPrice = cycle.FollowerEntryPrice
	}
	addCount := 0
	leaderPnL, leaderSize := float64(0), float64(0)
	if ti.engine != nil {
	...
	metadata := map[string]interface{}{
		"confirmation": "position_margin_stop_crossed", "quantity": attempt.Quantity,
		"trigger_price": stopPrice, "mark_price": observedPrice,
		"actual_order_id": attempt.ExitOrderID, "fee_observed": fee,
		"configured_margin_loss_pct":        attempt.StopConfiguredMarginLossPct,
		"current_effective_margin_loss_pct": attempt.ExpectedPositionLossPct,
	}
	begin := store.CopyGuardRiskExitBegin{
		CycleID: cycle.ID, TraderID: ti.traderID, LeaderPosID: cycle.LeaderPosID,
		AttemptNo: cycle.ReentryCount, TriggerPrice: stopPrice, Quantity: attempt.Quantity,
		TriggerSource: "verified_mark_crossing", LeaderPnL: leaderPnL, LeaderSize: leaderSize,
		AddCount: addCount, Metadata: metadata,
	}
	if err := ti.establishRiskExitGate(begin); err != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_BEGIN_FAILED", err, 0, false)
		return false
	}
	if _, err := ti.store.CopyTrade().FinalizeCopyGuardRiskExit(store.CopyGuardRiskExitFinalize{
		CopyGuardRiskExitBegin: begin, ExitPrice: fillPrice, Fee: fee,
		ActualOrderID: attempt.ExitOrderID, Observed: true, VenueState: "mark_crossed_exit",
	}); err != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionForcedExitPending, "POSITION_MARGIN_STOP_PERSIST_FAILED", err, 0, false)
		return false
	}
	return true
}
...
func (ti *TraderIntegration) unprotectableDisposition(cycle *store.CopyGuardCycle) string {
	// Ordinary leader execution is independent of protection availability.
	// AI reentry retains its explicit forceClose execution contract.
	return "warn"
}
...
func (ti *TraderIntegration) handleUnprotectableCycle(cycle *store.CopyGuardCycle, cause error, forceClose bool) string {
	...
	exitOrderID, execErr := ti.closeCopyGuardPosition(cycle)
	if execErr != nil {
		...
		if cycle.Status == store.CopyGuardReentryPending {
			...
			case "CANCELED", "REJECTED", "FAILED":
				...
				if candidate, candidateErr := ti.store.ReentryAI().GetReentryCandidateByCycle(cycle.ID); candidateErr == nil && candidate.Status == store.ReentryCandidateEntryPending {
					...
					if cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config); cycleConfig != nil {
						retry = cycleConfig.RiskAIMinReviewSeconds
					}
					if retry < 300 {
						retry = 300
					}
...
func (ti *TraderIntegration) recoverFilledReentry(cycle *store.CopyGuardCycle, order map[string]interface{}) error {
	...
	// This is a newly acknowledged AI attempt, not the released prior attempt.
	// Its own entry fills must prove continuity before custody can be rebound.
	orderID := getStringField(order, "orderId")
	if history, ok := ti.executor.(trader.SymbolTradeHistoryProvider); ok {
		start := cycle.UpdatedAt.Add(-2 * time.Second)
		if cycle.PendingSince != nil {
			start = cycle.PendingSince.Add(-2 * time.Second)
		}
		var fills []trader.TradeRecord
		var historyErr error
		if scoped, yes := ti.executor.(trader.ScopedTradeHistoryProvider); yes {
			fills, historyErr = scoped.GetTradesForPosition(cycle.Symbol, cycle.MarginMode, start)
		} else {
			fills, historyErr = history.GetTradesForSymbol(cycle.Symbol, start, 100)
		}
		if historyErr != nil {
			return historyErr
		}
		ended, proofErr := positionEndedInFills(fills, orderID, cycle.Symbol, cycle.Side)
		if proofErr != nil {
			return proofErr
		}
		if ended {
			return fmt.Errorf("reentry position has already ended")
		}
	}
	quantity, positionKnown := ti.followerPositionQuantity(cycle.Symbol, cycle.Side, cycle.MarginMode, cycle.FollowerPosID, true)
	if !positionKnown {
	...
	ti.queueProtectionRefresh(dec)
	if cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config); cycleConfig != nil && cycleConfig.RiskReentryDecisionMode == "ai_guarded" {
		ti.enforceAIReentryProtection(dec)
	}
	return nil
}
...
func (ti *TraderIntegration) executeFullDecision(fullDec *decision.FullDecision) {
	...
	for i := range fullDec.Decisions {
		...
		if accountLeaderExit(dec) {
			action, log := ti.recordAccountLeaderExit(dec)
			decisionActions = append(decisionActions, action)
			executionLogs = append(executionLogs, log)
			continue
		}
		if ti.engine.config.RiskPolicyVersion >= 4 && isAIReentryDecision(dec) {
		...
		if err == nil {
			err = ti.rejectPositionMarginReentryBeforeOrder(dec)
		}
		if err == nil && ti.usesAIGuardedLifecycle(dec) {
		...
		if err != nil {
			...
			reentryExecution := ti.engine.config.RiskPolicyVersion >= 4 && isAIReentryDecision(dec)
			...
			fixedReentryBlocked := reentryExecution && ReasonCodeOf(err) == "POSITION_MARGIN_REENTRY_BLOCKED"
			ambiguousReentry := reentryExecution && isAmbiguousReentryExecutionError(err)
			if !pendingReconciliation && reentryExecution && !deferredAIPriceLease && !fixedReentryBlocked {
			...
			if errors.Is(err, errCopyGuardRiskExitGate) {
				ti.finishGatedSource(dec, store.SourceConsumeNoFill, "STOPPED_BY_RISK", err)
				...
				ti.finishGatedSource(dec, store.SourceSupersedeNoFill, "MANUAL_STOP_FOLLOW", err)
				executionLogs = append(executionLogs, fmt.Sprintf("🖐️ %s %s 该仓位已手动停止跟单，忽略在途跟随动作", dec.Action, dec.Symbol))
				ti.saveSignalLog(dec, "skipped", err.Error())
			} else if ReasonCodeOf(err) == "CUSTODY_CHECK_PENDING" || ReasonCodeOf(err) == "OLD_PROTECTION_RETIRING" {
				ti.deferSourceTransitionRevalidation(dec, ReasonCodeOf(err), err)
				ti.saveSignalLog(dec, "reconciling", err.Error())
				executionLogs = append(executionLogs, "⏳ 原仓归属待核实，保留跟单指令等待重试")
			} else if ReasonCodeOf(err) == "INDEPENDENT_POSITION_CONFLICT" {
				if commitErr := ti.commitIgnoredOpenLeaderTransition(dec, "INDEPENDENT_POSITION_CONFLICT"); commitErr != nil {
					ti.deferSourceTransitionRevalidation(dec, "INDEPENDENT_POSITION_CONFLICT", commitErr)
				} else {
					dec.ExecutionStatus = store.ExecutionIntentSkipped
					dec.ExecutionReasonCode = "INDEPENDENT_POSITION_CONFLICT"
				}
				ti.saveSignalLog(dec, "skipped", err.Error())
				executionLogs = append(executionLogs, "🖐️ 同范围已有独立手动仓，本轮不接管")
			} else if ReasonCodeOf(err) == "POSITION_CUSTODY_RELEASED" {
				if commitErr := ti.store.CopyTrade().CommitDetachedLeaderTransition(dec.ExecutionIntentID, ti.traderID, dec.LeaderPosID, dec.SourceRevision, dec.LeaderPosSize, "POSITION_CUSTODY_RELEASED"); commitErr != nil {
					ti.deferSourceTransitionRevalidation(dec, "POSITION_CUSTODY_RELEASED", commitErr)
				} else {
					dec.ExecutionStatus, dec.ExecutionReasonCode = store.ExecutionIntentSkipped, "POSITION_CUSTODY_RELEASED"
				}
				if c, e := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID); e == nil {
					// A source member losing custody does not prove that its
					// shared physical protection scope is flat.
					if continuityErr := ti.verifyCopyGuardContinuity(c); errors.Is(continuityErr, errCopyPositionEnded) {
						_ = ti.store.CopyTrade().MarkCopyGuardFollowerAbsent(c.ID, ti.traderID, dec.LeaderPosID)
					} else if continuityErr != nil {
						ti.recordProtectionFollowup(dec, "PROTECTION_CONTINUITY_PENDING", continuityErr)
					}
				}
				ti.saveSignalLog(dec, "skipped", err.Error())
				executionLogs = append(executionLogs, "🖐️ 原跟单仓已结束，忽略旧周期指令")
				...
				if cycle, cycleErr := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID); cycleErr == nil {
					_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
						CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_REENTRY_BLOCKED",
						Metadata: map[string]interface{}{"attempt_no": cycle.ReentryCount, "execution_intent_id": dec.ExecutionIntentID, "reason": err.Error()},
					})
				}
				executionLogs = append(executionLogs, fmt.Sprintf("🛑 %s %s 固定仓位止损周期禁止二次进场", dec.Action, dec.Symbol))
				ti.saveSignalLog(dec, "skipped", err.Error())
			} else if deferredAIPriceLease {
				...
				ti.saveSignalLog(dec, "reconciling", err.Error())
			} else if ti.shouldReplayPreSubmitFailure(dec, err) {
				...
				if ti.benignCloseConfirmedFlat(dec) {
					if ti.handleBenignCloseFailure(dec, err) {
						...
						executionLogs = append(executionLogs,
							fmt.Sprintf("⏳ %s %s 空仓已确认，但本地周期收尾未完成并等待重试: %v",
								dec.Action, dec.Symbol, err))
					}
				} else {
				...
				sourceHandled := ti.finishFailedLeaderSource(dec, err)
				if !sourceHandled {
...
func (ti *TraderIntegration) recordAIReentrySubmitted(dec *decision.Decision) {
	if dec == nil || ti.store == nil || ti.engine == nil || ti.engine.config == nil || !strings.Contains(dec.Reasoning, "reentry") {
		return
	}
	cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID)
	if err != nil {
	...
	if cfg := copyGuardLifecycleConfig(cycle, ti.engine.config); cfg == nil || cfg.RiskReentryDecisionMode != "ai_guarded" {
		return
	}
...
func (ti *TraderIntegration) commitAcknowledgedLeaderExecution(dec *decision.Decision) error {
	...
	if dec.CopyTradeAction == "" && (dec.Action == "open_long" || dec.Action == "open_short") {
		dec.CopyTradeAction = "open"
		if isAdd {
			dec.CopyTradeAction = "add"
		}
	}
	...
	if !isAdd && (dec.Action == "open_long" || dec.Action == "open_short") &&
		usesV4CopyGuardRisk(ti.engine.config) {
		commit.InitialCopyGuard = &store.InitialCopyGuardLifecycle{
			PolicySnapshot:   ti.policySnapshot,
			LeaderEntryPrice: dec.EntryPrice,
			AccountEquity:    ti.executionEquityEvidence(dec),
			ATRAtEntry:       ti.copyGuardEntryATR(dec.Symbol, dec.EntryPrice),
		}
	}
...
// shouldReplayPreSubmitFailure preserves durable pre-submit retries without
// shadowing a benign risk-reducing close. The latter must first pass the fresh
// flat-position confirmation path; otherwise a wrapped "position not found"
// repeats forever even though the leader has already closed.
func (ti *TraderIntegration) shouldReplayPreSubmitFailure(dec *decision.Decision, err error) bool {
	return ReasonCodeOf(err) == "PRE_SUBMIT" && !ti.isBenignCloseError(dec, err)
}
...
func (ti *TraderIntegration) executeDecisionWithRetry(dec *decision.Decision) error {
	...
	err := ti.executeDecisionUnderRiskExitGate(dec, executionDecision)
...
func (ti *TraderIntegration) bindExecutionAttemptRecorder(dec *decision.Decision) {
	...
	dec.OnLeverageConfirmed = func(clientID string, leverage int) {
		if err := ti.store.CopyTrade().RecordCopyGuardLeverageReceipt(intentID, clientID, leverage); err != nil {
			// Audit failure must not veto the leader's order. No future weighted
			// average will be accepted as substitute evidence.
			logger.Errorf("[CopyGuard] leverage receipt persistence failed intent=%d: %v", intentID, err)
		}
	}
	...
	dec.BeforeExchangeSubmit = func(clientOrderID string) error {
		...
		if err := ti.store.CopyTrade().CheckFollowSubmission(intentID); err != nil {
			if errors.Is(err, store.ErrFollowControlChanged) {
				return fmt.Errorf("%w: %v", errManualStopFollowGate, err)
			}
			return reasonError("PRE_SUBMIT", "read durable follow control: %w", err)
		}
...
func (ti *TraderIntegration) confirmExecutionFill(dec *decision.Decision) error {
	...
	ti.observeExecutionFillTime(dec.ExecutionIntentID, order)
	state := strings.ToUpper(getStringField(order, "status", "state"))
	filled := getFloatField(order, "executedQty", "filled_quantity", "quantity")
...
func (ti *TraderIntegration) handleBenignCloseFailure(dec *decision.Decision, err error) bool {
	traderName := ti.traderDisplayName()
	...
	if ti.store != nil && dec.LeaderPosID != "" {
		...
		// 未来的同币种仓位上误触发。确认存在周期时，周期与 mapping 必须
		// 在同一事务中终止；任一步失败都保留 active mapping 供下一轮重试。
		mappingClosed := false
		if ti.engine != nil && ti.engine.config != nil && SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 {
			cycleFound, finalizeErr := ti.finalizeCopyGuardCycleState(dec, true)
			if finalizeErr != nil {
				if ti.engine != nil {
					ti.engine.UnmarkDecisionSources(dec)
				}
				ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "FLAT_TERMINATION_PENDING", finalizeErr.Error())
				logger.Warnf("⚠️ [%s] 空仓已确认但 Copy Guard 周期/mapping 原子收尾失败，保留重试: %v", ti.traderID, finalizeErr)
				ti.saveSignalLog(dec, "reconciling", finalizeErr.Error())
				return false
			}
			mappingClosed = cycleFound
		}
		// 没有 Copy Guard 周期时复用普通 CloseMapping；有周期时联合事务已经
		// 完成 mapping 关闭，不再执行第二次独立写入。
		if !mappingClosed {
			if cerr := ti.store.CopyTrade().CloseMapping(ti.traderID, dec.LeaderPosID, dec.EntryPrice); cerr != nil {
				...
				ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, "FLAT_TERMINATION_PENDING", cerr.Error())
				logger.Warnf("⚠️ [%s] 关闭映射失败（保留对账重试）: %v", ti.traderID, cerr)
				ti.saveSignalLog(dec, "reconciling", cerr.Error())
				return false
			}
		}
...
func (ti *TraderIntegration) validateAIReentryImmediatelyBeforeOrder(dec *decision.Decision) error {
	...
	_, persistedErr := ti.store.CopyTrade().GetByTraderID(ti.traderID)
	if persistedErr != nil {
	...
	cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
	if cycleConfig == nil || cycleConfig.RiskProtectionMode == store.RiskProtectionModePositionMarginPct ||
		!cycleConfig.RiskReentryEnabled || cycleConfig.RiskReentryDecisionMode != "ai_guarded" {
		return aiReentryPreflightError("EXECUTION_DISABLED", "lifecycle policy forbids AI reentry")
	}
	...
	if cycle.ReentryCount >= cycleConfig.RiskMaxReentries {
...
func (ti *TraderIntegration) recordActionEvent(dec *decision.Decision, status, errorMsg, signalID string) {
	...
	// The mapping has already changed at this point. Classify the immutable
	// source action, never infer the past from its post-commit active state.
	if eventType == store.CopyEventTypeOpen && (strings.EqualFold(dec.CopyTradeAction, "add") ||
		(dec.CopyTradeAction == "" && strings.Contains(strings.ToLower(dec.Reasoning), "add"))) {
	...
	case "partially_filled", "reconciling":
		evStatus, severity = status, store.CopyEventSeverityWarn
	default:
		evStatus, severity = "unknown", store.CopyEventSeverityWarn
	}
	...
	if timing, err := ti.store.CopyTrade().GetExecutionTiming(dec.ExecutionIntentID); err == nil {
		if detail == nil {
			detail = map[string]interface{}{}
		}
		detail["timing"] = timing
	}
	if intent, err := ti.store.CopyTrade().GetExecutionIntentByID(dec.ExecutionIntentID); err == nil {
		if detail == nil {
			detail = map[string]interface{}{}
		}
		detail["requested_quantity"], detail["quantized_quantity"] = intent.RequestedQuantity, intent.QuantizedQuantity
		detail["quantity_step"] = intent.QuantityStep
		detail["filled_quantity"], detail["filled_notional"] = intent.FilledQuantity, intent.FilledNotional
		detail["residual_quantity"] = math.Max(0, intent.TargetQuantity-intent.FilledQuantity)
		detail["notional_gap"] = math.Max(0, intent.RequestedNotional-intent.FilledNotional)
		detail["reason_code"], detail["catchup_reason"] = intent.ReasonCode, intent.LastCatchupReason
	}
	ti.recordCopyEvent(&store.CopyTradeEvent{
...
func buildActionDetail(dec *decision.Decision, errorMsg string) map[string]interface{} {
	...
	if dec.CopyLeaderEquity > 0 {
		d["leader_equity"] = dec.CopyLeaderEquity
		d["follower_equity"] = dec.CopyFollowerEquity
		d["copy_coefficient"] = dec.CopyCoefficient
		d["source_notional"] = dec.SourceNotional
		d["target_notional"] = dec.CopyFollowerEquity * dec.CopyCoefficient * dec.SourceNotional / dec.CopyLeaderEquity
	}
	if dec.SourceSnapshotMS > 0 {
		d["source_snapshot_ms"] = dec.SourceSnapshotMS
	}
	if dec.SignalObservedMS > 0 {
		d["signal_observed_ms"] = dec.SignalObservedMS
	}
	if dec.LeaderExitScope != "" {
		d["leader_exit_scope"] = dec.LeaderExitScope
	}
...
func (ti *TraderIntegration) commitCopyGuardReentryFill(dec *decision.Decision) error {
	...
	if dec.ExecutionIntentID > 0 {
		...
		if result.FirstFill {
			ti.markManualReentryOutcome(cycle.ID, store.ManualReentryStatusExecuted, "")
			ti.clearRiskExitGate(cycle.LeaderPosID, cycle.ID)
		}
...
func (ti *TraderIntegration) handleReentryLifecycleCommitFailure(dec *decision.Decision, cause error) {
	...
	exitScope, exitQuantity, scopeErr := ti.uncommittedReentryExitScope(dec, cycle)
	if scopeErr != nil {
		metadata["forced_exit_error"] = scopeErr.Error()
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "GUARD_FORCED_EXIT_FAILED", Metadata: metadata})
		ti.notifyProtection(cycle, "AI 重入异常仓位归属待核实", reason+"；"+scopeErr.Error(), fmt.Sprintf("reentry_commit_scope_unknown_%d", attemptNo))
		return
	}
	if _, closeErr := ti.closeCopyGuardPositionWithQuantity(exitScope, exitQuantity, false); closeErr != nil {
	...
	if qty, known := exitQuantity(); !known || qty > 0 {
	...
	if err := ti.store.CopyTrade().FinalizeUncommittedReentryExit(cycle.ID, attemptNo); err != nil {
		return
	}
...
func (ti *TraderIntegration) copyGuardEntryATR(symbol string, entryPrice float64) float64 {
	...
	cfg := ti.engine.config
	atr, _ := market.CachedOKXATR(symbol, cfg.RiskATRTimeframe, cfg.RiskATRPeriod, riskATRCacheMaxAge(cfg))
...
func (ti *TraderIntegration) updatePositionMapping(dec *decision.Decision, committed ...bool) {
	if ti.riskExitGateActive(dec) {
		return // Late fills were audited; only the pending-exit monitor owns them.
	}
	...
	case "open_long", "open_short":
		...
		if existingMapping != nil && existingMapping.Side != expectedSide {
			...
			if SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 {
				oldCycle, cycleErr := ti.protectionCycleForDecision(dec)
				if cycleErr == nil {
					...
					if shadowErr := ti.finalizeCopyGuardPositionMarginShadow(oldCycle.ID, dec.EntryPrice); shadowErr != nil {
						logger.Warnf("⚠️ [%s] 80%% 影子评测闭合失败 | cycle=%d: %v", ti.traderID, oldCycle.ID, shadowErr)
					}
		...
		if mappingAlreadyCommitted {
			if SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 && (ti.engine.config.RiskStopLossEnabled || ti.engine.config.RiskLiquidationGuardEnabled) {
				_, cycleErr := ti.protectionCycleForDecision(dec)
				if errors.Is(cycleErr, sql.ErrNoRows) {
					// A current execution intent creates mapping + cycle + attempt 0 +
					// initial-fill event in CommitLeaderExecutionFill. If an older row
					// lacks that atomic ledger, recover only from durable first-fill
					// evidence; never manufacture INITIAL_ENTRY_FILLED from today's
					// weighted-average position.
					cycleErr = nil
					if existingMapping == nil {
						existingMapping, cycleErr = copyTradeStore.GetActiveMapping(ti.traderID, dec.LeaderPosID)
					}
					if cycleErr == nil && existingMapping != nil {
						_, _, cycleErr = ti.ensureV4CycleForMapping(existingMapping, nil, "post-fill lifecycle recovery")
					}
				}
				if cycleErr != nil {
					logger.Warnf("⚠️ [%s] 恢复 Copy Guard 生命周期失败: %v", ti.traderID, cycleErr)
					ti.recordProtectionFollowup(dec, "COPY_GUARD_CYCLE_RECOVERY_PENDING", cycleErr)
				}
			}
			...
			if err := copyTradeStore.SavePositionMapping(mapping); err != nil {
				...
				if SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 && (ti.engine.config.RiskStopLossEnabled || ti.engine.config.RiskLiquidationGuardEnabled) {
					// No execution intent means there is no durable proof that this
					// callback represents the immutable first fill. Keep following, but
					// recover it as LEGACY_UNVERIFIED so fixed anchoring and scoring stay
					// disabled for this lifecycle.
					if _, _, recoverErr := ti.ensureV4CycleForMapping(mapping, nil, "post-fill callback has no execution intent"); recoverErr != nil {
						logger.Warnf("⚠️ [%s] 恢复未验证 Copy Guard 生命周期失败: %v", ti.traderID, recoverErr)
					}
				}
			}
		}
		if SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 && dec.ExecutionIntentID > 0 && dec.ExchangeOrderID != "" {
			if cycle, cycleErr := ti.protectionCycleForDecision(dec); cycleErr == nil {
	...
	case "close_long", "close_short":
		if SupportsCopyGuard(ti.engine.config.ProviderType) && ti.engine.config.RiskPolicyVersion >= 4 {
			if mappingAlreadyCommitted {
				// The business fill is already final. Its first protection
				// callback has the same separation as subsequent retries.
				protectionDec := *dec
				protectionDec.ExecutionIntentID = 0
				if _, err := ti.finalizeCopyGuardCycleState(&protectionDec, false); err != nil {
					ti.recordProtectionFollowup(dec, "COMPLETED_EXIT_PROTECTION_PENDING", err)
				} else {
					_ = copyTradeStore.ResolveRuntimeIssue(ti.traderID, "protection", dec.LeaderPosID)
				}
			} else {
				ti.finalizeCopyGuardCycle(dec)
			}
...
func (ti *TraderIntegration) finalizeCopyGuardCycle(dec *decision.Decision) {
	_, _ = ti.finalizeCopyGuardCycleState(dec, false)
}

// finalizeCopyGuardCycleState performs the exchange-side protection cleanup and
// lifecycle accounting transition. When closeMapping is true, the final
// lifecycle transition and mapping close share one store transaction; this is
// required by the confirmed-flat benign-close recovery path.
func (ti *TraderIntegration) finalizeCopyGuardCycleState(dec *decision.Decision, closeMapping bool) (bool, error) {
	if ti == nil || ti.store == nil || dec == nil || dec.LeaderPosID == "" {
		return false, fmt.Errorf("invalid Copy Guard cycle finalization")
	}
	if !ti.protectionAsync.Load() {
		ti.protectionMu.Lock()
		defer ti.protectionMu.Unlock()
	}
	...
	if keep, groupErr := ti.store.CopyTrade().FollowGroupKeepsProtection(cycle.ID); groupErr != nil {
		return false, groupErr
	} else if keep {
		// Source-member completion (or a paused source ending) does not end
		// the actual position protected by this immutable cycle.
		return false, nil
	}
	if !ti.protectionAsync.Load() {
		order, orderErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID)
	...
	leaderAccountScope := dec.IsCopyTrade && dec.LeaderExitScope == leaderAccountExitScope
	if leaderAccountScope && closePrice > 0 {
		baselineSource = "source_snapshot_mark"
		if dec.LeaderReversed {
			baselineSource = "leader_reversal_mark"
		}
	}
	if closePrice <= 0 && ti.engine != nil && !leaderAccountScope {
	...
	if shadowErr := ti.finalizeCopyGuardPositionMarginShadow(cycle.ID, closePrice); shadowErr != nil {
		logger.Warnf("⚠️ [%s] 80%% 影子评测闭合失败 | cycle=%d: %v", ti.traderID, cycle.ID, shadowErr)
	}
	...
	if closeMapping {
		err = ti.store.CopyTrade().BeginCopyGuardAccountingAndCloseMapping(
			cycle.ID, ti.traderID, dec.LeaderPosID, terminalStatus, dec.ExchangeOrderID, baseline, closePrice,
		)
	} else {
		err = ti.store.CopyTrade().BeginCopyGuardAccounting(cycle.ID, terminalStatus, dec.ExchangeOrderID, baseline)
	}
	if err != nil {
...
func (ti *TraderIntegration) captureV4FollowerBeforeClose(dec *decision.Decision) {
	...
	if !ti.copyGuardOwnsPosition(cycle) || ti.verifyCopyGuardContinuity(cycle) != nil {
		return
	}
	positions, err := ti.getFreshPositions()
	if err != nil {
...
func (ti *TraderIntegration) shouldManageStopLoss(dec *decision.Decision) bool {
	if ti.riskExitGateActive(dec) {
		return false // Never re-arm an already triggered lifecycle after a late fill.
	}
	if ti.engine == nil || ti.engine.config == nil {
		return false
	}
	...
	if !(ti.engine.config.RiskStopLossEnabled || ti.engine.config.RiskLiquidationGuardEnabled) {
		if dec.LeaderPosID == "" || ti.store == nil {
			return false
		}
		if _, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID); err != nil {
			return false
		}
	}
	return true
...
// performed. MARK_PRICE triggers may only use that value or the authoritative
// mark-price provider; last price is intentionally forbidden because a local
// false crossing would submit a real reduce-only market exit.
func (ti *TraderIntegration) stopTriggerReferencePrice(symbol string, positionMarkPrice float64) (float64, string) {
	...
	return ti.stopTriggerReferencePriceForType(symbol, positionMarkPrice, triggerType)
}

func (ti *TraderIntegration) stopTriggerReferencePriceForType(symbol string, positionMarkPrice float64, triggerType string) (float64, string) {
	if strings.EqualFold(triggerType, "mark") {
		if positionMarkPrice > 0 {
			return positionMarkPrice, "mark_position"
		}
		if priceProvider, ok := ti.executor.(trader.MarkPriceProvider); ok {
			if current, err := priceProvider.GetMarkPrice(symbol); err == nil && current > 0 {
				return current, "mark_endpoint"
			}
		}
		return 0, ""
	}
...
func (ti *TraderIntegration) recordPositionMarginMarkUnavailable(cycle *store.CopyGuardCycle, symbol, phase string, cause error) {
	if ti == nil || cycle == nil {
		return
	}
	detail := "exchange mark price is unavailable"
	if cause != nil {
		detail = cause.Error()
	}
	key := fmt.Sprintf("attempt=%d phase=%s", cycle.ReentryCount, phase)
	if !ti.shouldRecordProtectionEvent(cycle.ID, "POSITION_MARGIN_MARK_UNAVAILABLE", key) {
		return
	}
	_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
		CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_MARK_UNAVAILABLE",
		Metadata: map[string]interface{}{
			"attempt_no": cycle.ReentryCount, "symbol": symbol, "phase": phase,
			"reason": detail, "last_price_fallback_forbidden": true,
		},
	})
	logger.Errorf("🚨 [%s] fixed Copy Guard 缺少权威标记价，禁止用最新成交价判断穿越 | cycle=%d %s phase=%s reason=%s",
		ti.traderID, cycle.ID, symbol, phase, detail)
	ti.notifyProtection(cycle, "Copy Guard 标记价暂不可用",
		"固定比例止损不会使用最新成交价代替标记价，也不会据此主动市价平仓；系统将保留现有交易所托管保护并持续重试。原因: "+detail,
		fmt.Sprintf("position_margin_mark_unavailable_%d_%s", cycle.ReentryCount, phase))
}
...
func (ti *TraderIntegration) refreshStopLossAfterExecute(dec *decision.Decision) {
	...
	defer ti.protectionMu.Unlock()
	positions, err := ti.getFreshPositions()
	ti.refreshStopLossAfterExecuteLocked(dec, positions, err)
}

// refreshStopLossAfterExecuteLocked is the common planner for synchronous
// position transitions and the periodic fixed-stop safety sweep. The caller
// must hold protectionMu and supply one authoritative position snapshot. This
// lets a 15-second sweep revalidate every active cycle with a single exchange
// request instead of one request per position.
func (ti *TraderIntegration) refreshStopLossAfterExecuteLocked(dec *decision.Decision, positions []map[string]interface{}, positionsErr error) {
	if !ti.shouldManageStopLoss(dec) {
		return
	}
	var err error

	if c, err := ti.protectionCycleForDecision(dec); err == nil && !ti.copyGuardOwnsPosition(c) {
		if stored, readErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(c.ID); readErr == nil {
			if mgr, ok := ti.executor.(ProtectiveStopManagerV4); ok {
				_ = ti.cancelProtectiveOrderForCycle(mgr, c, stored)
			}
		}
		return
	}
	...
	// 拿最新持仓
	if positionsErr != nil {
		logger.Warnf("⚠️ [%s] 重挂 SL 失败（GetPositions 错误）: %v | %s", ti.traderID, positionsErr, dec.Symbol)
		ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "PROTECTION_VERIFY_UNKNOWN", positionsErr, 0)
		return
	}
	...
	if matchedPos == nil {
		if cycle, err := ti.protectionCycleForDecision(dec); err == nil {
			if cycle.ProtectionStatus == store.CopyGuardProtectionPositionAbsent || ti.confirmCopyGuardFollowerAbsent(cycle) {
				return
			}
		}
	...
	cycle, cycleErr := ti.protectionCycleForDecision(dec)
	if cycleErr != nil {
		logger.Errorf("❌ [%s] Copy Guard lifecycle missing before stop planning: %v", ti.traderID, cycleErr)
		ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "PROTECTION_CYCLE_UNAVAILABLE", cycleErr, 0)
		return
	}
	cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
	if cycleConfig == nil {
		ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "PROTECTION_POLICY_UNAVAILABLE", fmt.Errorf("lifecycle policy unavailable"), 0)
		return
	}
	if err := ti.verifyCopyGuardContinuity(cycle); err != nil {
		if !errors.Is(err, errCopyPositionEnded) {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "POSITION_CONTINUITY_UNVERIFIED", err, cycle.ProtectionCoverage, false)
		}
		return
	}
	if cycle.ProtectionStatus == store.CopyGuardProtectionFlatReconciling {
		// A real, continuous residual has appeared. Protection is needed again;
		// errors/empty snapshots alone must never undo a confirmed flat fact.
		if err := ti.store.CopyTrade().UpdateCopyGuardProtectionHealth(cycle.ID, store.CopyGuardProtectionPending, 0, "continuous residual position observed", cycle.FollowerPosID, cycle.EntryOrderID, false); err != nil {
			return
		}
		cycle.ProtectionStatus = store.CopyGuardProtectionPending
	}
	if err := ti.reconcileManualStop(cycle, quantity, 0); err != nil {
		if !errors.Is(err, errCopyGuardRiskExitGate) {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "MANUAL_STOP_RECONCILING", err, cycle.ProtectionCoverage, false)
		}
		return
	}
	stopControl, controlErr := ti.store.CopyTrade().GetCopyGuardStopControl(cycle.ID, cycle.ReentryCount)
	if controlErr != nil {
		return
	}
	if liquidationOnly(cycleConfig) {
		ti.refreshLiquidationOnly(dec, cycle, matchedPos, stopControl.ManualPrice)
		return
	}
	if cycleConfig.RiskLiquidationGuardEnabled && (invalidPositive(liquidationPrice) || invalidPositive(positionMarkPrice) || (expectedSide == "long" && liquidationPrice > positionMarkPrice) || (expectedSide == "short" && liquidationPrice < positionMarkPrice)) {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "LIQUIDATION_SAFETY_UNVERIFIED", fmt.Errorf("fresh liquidation and mark prices unavailable"), cycle.ProtectionCoverage, false)
		return
	}
	protectionMode, positionMarginPct := copyGuardProtectionSettings(cycle, cycleConfig)
	fixedPositionMargin := protectionMode == store.RiskProtectionModePositionMarginPct
	if fixedPositionMargin && (cycle.ReentryCount != 0 || isAIReentryDecision(dec)) {
		err := fmt.Errorf("position_margin_pct lifecycle forbids reentry attempt %d", cycle.ReentryCount)
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_REENTRY_BLOCKED", Metadata: map[string]interface{}{"attempt_no": cycle.ReentryCount, "reason": err.Error()}})
		ti.transitionExecutionIntent(dec, store.ExecutionIntentFailed, "POSITION_MARGIN_REENTRY_BLOCKED", err.Error())
		return
	}

	followerEquity := ti.getEquityFunc()()
	if followerEquity <= 0 && !fixedPositionMargin {
		if cycleConfig.RiskLiquidationGuardEnabled {
			ti.refreshLiquidationOnly(dec, cycle, matchedPos, stopControl.ManualPrice)
			return
		}
	...
	if fixedPositionMargin {
		effectiveStopPct = 0
		stopPctSource = "not_applicable_position_margin_pct"
	} else if policy, policyErr := ti.store.CopyTrade().EffectiveCopyGuardStopPolicy(ti.traderID, cycleConfig.RiskStopMaxAccountLossPct); policyErr == nil {
	...
	slInput := &StopLossCalcInput{
		...
		StructureInvalidation: 0,
	}
	if !fixedPositionMargin {
		slInput.StructureInvalidation = structuralInvalidationPrice(dec.Symbol, side, entryPrice)
	}
	...
	if !fixedPositionMargin {
		ti.syncPositionMarginStopShadow(dec, cycle, side, entryPrice, quantity, leverage, slInput.PriceTickSize, slInput.BaseQuantityStep, positionMarkPrice)
	}

	var slResult *StopLossCalcResult
	var fixedAnchor *store.CopyGuardStopAnchor
	var fixedAnchorPlan *PositionMarginStopAnchorPlan
	var fixedMarkObservation trader.MarkPriceObservation
	var fixedEvaluation *PositionMarginStopEvaluation
	fixedExistingQuantity := float64(0)
	if fixedPositionMargin {
		if slInput.PriceTickSize <= 0 || slInput.BaseQuantityStep <= 0 {
			cause := fmt.Errorf("position_margin_pct requires exact execution tick and quantity precision")
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "PROTECTION_PRECISION_UNAVAILABLE", cause, 0)
			return
		}
		fixedAnchor, err = ti.store.CopyTrade().GetCopyGuardStopAnchor(cycle.ID, cycle.ReentryCount)
		created := false
		if errors.Is(err, sql.ErrNoRows) {
			evidence, evidenceErr := ti.store.CopyTrade().GetCopyGuardInitialFillEvidence(cycle.ID)
			if evidenceErr == nil && evidence.Leverage <= 0 && dec.ExecutionIntentID == cycle.InitialIntentID {
				mapping, mappingErr := ti.store.CopyTrade().GetMapping(ti.traderID, cycle.LeaderPosID)
				if mappingErr == nil && positionMarginAnchorInitializationAllowed(dec, mapping, cycle.ReentryCount) {
					evidenceErr = ti.store.CopyTrade().CompleteInitialFillLeverage(cycle.ID, cycle.InitialIntentID, float64(leverage))
					if evidenceErr == nil {
						evidence, evidenceErr = ti.store.CopyTrade().GetCopyGuardInitialFillEvidence(cycle.ID)
					}
				}
			}
			if evidenceErr != nil || evidence.IntentID != cycle.InitialIntentID || evidence.Leverage <= 0 {
				ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_ANCHOR_MISSING", fmt.Errorf("verified immutable initial fill evidence unavailable: %v", evidenceErr), 0)
				return
			}
			anchorPlan, planErr := BuildPositionMarginStopAnchor(side, evidence.EntryPrice, evidence.Quantity, evidence.Leverage, positionMarginPct, slInput.PriceTickSize)
			if planErr != nil {
				ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionDegraded, "POSITION_MARGIN_ANCHOR_INVALID", planErr, 0)
				return
			}
			fixedAnchorPlan = anchorPlan
			fixedAnchor, created, err = ti.store.CopyTrade().InitializeCopyGuardStopAnchor(cycle.ID, cycle.ReentryCount, store.CopyGuardStopAnchor{
				EntryPrice: anchorPlan.EntryPrice, Leverage: anchorPlan.Leverage,
				InitialMargin: anchorPlan.InitialMargin, Price: anchorPlan.StopPrice,
				ConfiguredMarginLossPct: anchorPlan.ConfiguredMarginLossPct,
				Source:                  store.CopyGuardAnchorSourceInitialFill,
				SourceIntentID:          evidence.IntentID,
			})
		}
		if err != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_ANCHOR_PERSIST_FAILED", err, 0)
			return
		}
		if created {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
				CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_STOP_ANCHOR_FROZEN",
				Price: fixedAnchor.Price, Quantity: quantity, Notional: entryPrice * quantity,
				Metadata: map[string]interface{}{
					"attempt_no": cycle.ReentryCount, "stop_anchor_entry_price": fixedAnchor.EntryPrice,
					"stop_anchor_leverage": fixedAnchor.Leverage, "stop_anchor_initial_margin": fixedAnchor.InitialMargin,
					"stop_anchor_price": fixedAnchor.Price, "configured_margin_loss_pct": fixedAnchor.ConfiguredMarginLossPct,
					"configured_risk_usd":            fixedAnchorPlan.ConfiguredRiskUSD,
					"raw_formula_stop_price":         fixedAnchorPlan.RawStopPrice,
					"aligned_price_risk_usd":         fixedAnchorPlan.AlignedPriceRiskUSD,
					"equivalent_price_move_pct":      fixedAnchorPlan.EquivalentPriceMovePct,
					"fees_funding_slippage_excluded": true,
				},
			})
		}
		existingStop := float64(0)
		if stored, storedErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID); storedErr == nil {
			existingStop = stored.TriggerPrice
			fixedExistingQuantity = stored.Quantity
		}
		if durableStop, durableErr := ti.store.CopyTrade().GetCopyGuardAttemptFinalStop(cycle.ID, cycle.ReentryCount); durableErr == nil {
			keepDurable := existingStop <= 0 ||
				(side == SideLong && durableStop > existingStop) ||
				(side == SideShort && durableStop < existingStop)
			if keepDurable {
				existingStop = durableStop
			}
		}
		anchorPrice := fixedAnchor.Price
		if stopControl.ManualPrice > 0 {
			anchorPrice = stopControl.ManualPrice
			existingStop, _ = ti.store.CopyTrade().GetCopyGuardAttemptFinalStop(cycle.ID, cycle.ReentryCount)
		}
		markObservation := ti.fixedStopMarkObservation(dec.Symbol, positionMarkPrice)
		fixedMarkObservation = markObservation
		markPrice := markObservation.Price
		fixedEvaluationInput := PositionMarginStopEvaluationInput{
			Side: side, CurrentEntryPrice: entryPrice, CurrentQuantity: quantity,
			CurrentLeverage: float64(leverage), AnchorStopPrice: anchorPrice,
			ExistingStopPrice: existingStop, MarkPrice: markPrice,
			LiquidationPrice: liquidationPrice, PriceTickSize: slInput.PriceTickSize,
			BaseQuantityStep: slInput.BaseQuantityStep, ATRValue: cycle.ATRAtEntry,
			LiquidationBufferATR: copyGuardLiquidationBufferATR(cycle, cycleConfig),
			FollowerEquity:       followerEquity,
		}
		fixedEvaluation, err = EvaluatePositionMarginStop(fixedEvaluationInput)
		if err != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionDegraded, "POSITION_MARGIN_STOP_PLAN_FAILED", err, 0)
			return
		}
		// Persist the mutable position audit and the current effective stop before
		// any exchange action. In particular, an already-crossed first stop may
		// market-exit without ever entering upsertV4Protection; recovery must still
		// know the exact tightened trigger after a process restart. The store update
		// is an atomic MAX (long) / MIN (short), so another process or an overlapping
		// recovery can never replace a tighter stop with this caller's stale plan.
		if updateErr := ti.store.CopyTrade().UpdateCopyGuardFollowerPosition(cycle.ID, "", entryPrice, entryPrice*quantity); updateErr != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_STOP_AUDIT_FAILED", fmt.Errorf("persist current follower position: %w", updateErr), 0)
			return
		}
		if updateErr := ti.store.CopyTrade().UpdateCopyGuardAttemptPosition(cycle.ID, cycle.ReentryCount, entryPrice, entryPrice*quantity, quantity, cycle.ATRAtEntry); updateErr != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_STOP_AUDIT_FAILED", fmt.Errorf("persist current stop attempt position: %w", updateErr), 0)
			return
		}
		durableStop, auditErr := ti.store.CopyTrade().TightenCopyGuardPositionMarginStopAuditWithMark(
			cycle.ID, cycle.ReentryCount, expectedSide, float64(leverage),
			fixedEvaluation.CurrentMargin, entryPrice, markPrice, fixedEvaluation.StopPrice,
			fixedEvaluation.GovernedBy, markObservation.ObservedAt,
		)
		if auditErr != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_STOP_AUDIT_FAILED", auditErr, 0)
			return
		}
		// Re-evaluate all display/trigger facts from the durable winner. This is
		// normally identical to the candidate, but is essential when a concurrent
		// process has already persisted a safer liquidation clamp.
		fixedEvaluationInput.ExistingStopPrice = durableStop
		fixedEvaluation, err = EvaluatePositionMarginStop(fixedEvaluationInput)
		if err != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionDegraded, "POSITION_MARGIN_STOP_PLAN_FAILED", err, 0)
			return
		...
		slResult = &StopLossCalcResult{
			SLPrice: fixedEvaluation.StopPrice, ATRValue: cycle.ATRAtEntry,
			GovernedBy: fixedEvaluation.GovernedBy, TickSize: slInput.PriceTickSize,
			QuantityStep: slInput.BaseQuantityStep, ExpectedLossUSD: fixedEvaluation.CurrentRiskUSD,
			ExpectedLossPct:       fixedEvaluation.CurrentAccountLossPct,
			ExpectedMarginLossPct: fixedEvaluation.CurrentEffectiveMarginLossPct,
			DistanceATRRatio:      fixedEvaluation.DistanceATRRatio, Clamped: fixedEvaluation.Clamped,
			LiquidationPriceIgnored: fixedEvaluation.LiquidationPriceIgnored,
		}
		if quantity > 0 {
			slResult.SLDistance = fixedEvaluation.CurrentRiskUSD / quantity
		}
		if fixedEvaluation.AlreadyCrossed {
			ti.handlePositionMarginStopCrossed(dec, cycle, expectedSide, quantity, markPrice, fixedEvaluation)
			return
		}
	} else if stopControl.ManualPrice > 0 {
		existingStop, _ := ti.store.CopyTrade().GetCopyGuardAttemptFinalStop(cycle.ID, cycle.ReentryCount)
		manualPlan, planErr := EvaluatePositionMarginStop(PositionMarginStopEvaluationInput{
			Side: side, CurrentEntryPrice: entryPrice, CurrentQuantity: quantity, CurrentLeverage: float64(leverage),
			AnchorStopPrice: stopControl.ManualPrice, ExistingStopPrice: existingStop, LiquidationPrice: liquidationPrice, MarkPrice: positionMarkPrice,
			PriceTickSize: slInput.PriceTickSize, BaseQuantityStep: slInput.BaseQuantityStep, ATRValue: cycle.ATRAtEntry,
			LiquidationBufferATR: copyGuardLiquidationBufferATR(cycle, cycleConfig), FollowerEquity: followerEquity,
		})
		if planErr != nil {
			ti.markProtectionIssueForDecision(dec, store.CopyGuardProtectionUnknown, "MANUAL_STOP_PLAN_FAILED", planErr, 0)
			return
		}
		slResult = &StopLossCalcResult{SLPrice: manualPlan.StopPrice, SLDistance: math.Abs(entryPrice - manualPlan.StopPrice),
			ATRValue: cycle.ATRAtEntry, GovernedBy: "manual_exchange", TickSize: slInput.PriceTickSize, QuantityStep: slInput.BaseQuantityStep,
			Clamped: manualPlan.Clamped, ExpectedLossUSD: manualPlan.CurrentRiskUSD, ExpectedLossPct: manualPlan.CurrentAccountLossPct, ExpectedMarginLossPct: manualPlan.CurrentEffectiveMarginLossPct}
	} else if isAIReentryDecision(dec) {
		if dec.StopLoss <= 0 {
		...
		if atr <= 0 {
			atr, _ = market.GetOKXATRWithMaxAge(dec.Symbol, cycleConfig.RiskATRTimeframe, cycleConfig.RiskATRPeriod, riskATRCacheMaxAge(cycleConfig))
			if atr <= 0 {
				atr = entryPrice * cycleConfig.RiskATRFallbackPct
			}
		}
		usage, usageErr := ti.store.ReentryAI().GetCopyGuardRiskUsageExcludingAttempt(ti.traderID, cycle.ID, cycle.ReentryCount)
		availableRisk, capacityErr := AvailableCopyGuardRiskUSD(cycleConfig, followerEquity, usage)
		...
		aiPlan, planErr := BuildAIProtectionPlanFromStop(cycleConfig, AIProtectionPlanInput{
		...
		if cycleConfig.RiskLiquidationGuardEnabled {
			// Independent guard composes the fixed safety boundary below. ATR
			// remains the strategy input, not a second liquidation clamp.
			strategyInput := *slInput
			strategyInput.LiquidationPrice = 0
			slResult, err = calcStopLossPrice(cycleConfig, &strategyInput)
		} else {
			slResult, err = calcStopLossPrice(cycleConfig, slInput)
		}
	}
	if cycleConfig.RiskLiquidationGuardEnabled && !fixedPositionMargin {
		if err != nil || slResult == nil || slResult.SLPrice <= 0 {
			ti.refreshLiquidationOnly(dec, cycle, matchedPos, stopControl.ManualPrice)
			return
		}
		existing, _ := ti.store.CopyTrade().GetCopyGuardAttemptFinalStop(cycle.ID, cycle.ReentryCount)
		evaluation, safetyErr := EvaluatePositionMarginStop(PositionMarginStopEvaluationInput{
			Side: side, CurrentEntryPrice: entryPrice, CurrentQuantity: quantity, CurrentLeverage: float64(leverage),
			AnchorStopPrice: slResult.SLPrice, ExistingStopPrice: existing, LiquidationPrice: liquidationPrice, MarkPrice: positionMarkPrice,
			PriceTickSize: slInput.PriceTickSize, BaseQuantityStep: slInput.BaseQuantityStep, FollowerEquity: followerEquity,
		})
		if safetyErr != nil {
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "LIQUIDATION_SAFETY_UNVERIFIED", safetyErr, cycle.ProtectionCoverage, false)
			return
		}
		slResult.SLPrice, slResult.SLDistance = evaluation.StopPrice, math.Abs(entryPrice-evaluation.StopPrice)
		slResult.Clamped = slResult.Clamped || evaluation.Clamped
		if evaluation.Clamped {
			slResult.GovernedBy = "liquidation_guard"
		}
		if evaluation.AlreadyCrossed {
			if auditErr := ti.store.CopyTrade().UpdateCopyGuardAttemptRiskAudit(cycle.ID, cycle.ReentryCount, float64(leverage), evaluation.CurrentMargin, 0, 0, "", 0, evaluation.StopPrice, evaluation.CurrentEffectiveMarginLossPct, slResult.GovernedBy); auditErr != nil {
				return
			}
			ti.handlePositionMarginStopCrossed(dec, cycle, expectedSide, quantity, positionMarkPrice, evaluation)
			return
		}
	}
	if err != nil {
		logger.Warnf("⚠️ [%s] 重挂 SL：算法失败: %v | %s", ti.traderID, err, dec.Symbol)
		if usesV4CopyGuardRisk(cycleConfig) {
	...
	if cycle != nil {
		if !fixedPositionMargin && stopControl.ManualPrice <= 0 {
			if stored, storedErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID); storedErr == nil && stored.Status == "live" && stored.TriggerPrice > 0 {
				...
				if keepExisting {
					...
					friction := (cycleConfig.RiskSlippageBufferBPS + cycleConfig.RiskRoundTripFeeBPS) / 10000
		...
		// Throttled: the retry chain replans on every poll, so an unstable cycle
		// used to write one PROTECTION_PLAN row every 3 seconds.
		planSignature := fmt.Sprintf("%s|%d|%d", slResult.GovernedBy, protectionSignatureBucket(quantity), protectionSignatureBucket(slResult.SLPrice))
		if ti.shouldRecordProtectionEvent(cycle.ID, "PROTECTION_PLAN", planSignature) {
			metadata := map[string]interface{}{
				"protection_mode": protectionMode, "governed_by": slResult.GovernedBy,
				"max_account_loss_pct": effectiveStopPct, "max_account_loss_source": stopPctSource,
				"structure_invalidation": slInput.StructureInvalidation,
				...
				"min_stop_atr_ratio":       cycleConfig.RiskMinStopATRRatio,
			}
			if fixedPositionMargin && fixedAnchor != nil && fixedEvaluation != nil {
				metadata["configured_margin_loss_pct"] = fixedAnchor.ConfiguredMarginLossPct
				metadata["stop_anchor_entry_price"] = fixedAnchor.EntryPrice
				metadata["stop_anchor_leverage"] = fixedAnchor.Leverage
				metadata["stop_anchor_initial_margin"] = fixedAnchor.InitialMargin
				metadata["stop_anchor_price"] = fixedAnchor.Price
				metadata["current_entry_price"] = entryPrice
				metadata["current_quantity"] = quantity
				metadata["current_leverage"] = leverage
				metadata["current_margin"] = fixedEvaluation.CurrentMargin
				metadata["current_risk_usd"] = fixedEvaluation.CurrentRiskUSD
				metadata["current_effective_margin_loss_pct"] = fixedEvaluation.CurrentEffectiveMarginLossPct
				metadata["equivalent_price_move_pct"] = fixedEvaluation.EquivalentPriceMovePct
				metadata["requested_stop_price"] = fixedEvaluation.StopPrice
				metadata["mark_source"] = fixedMarkObservation.Source
				metadata["mark_observed_at"] = fixedMarkObservation.ObservedAt
				metadata["liquidation_safety_tightened"] = fixedEvaluation.Clamped
				metadata["fees_funding_slippage_excluded"] = true
			}
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
		...
		if fixedPositionMargin && fixedEvaluation != nil {
			quantityChange := "unchanged"
			switch {
			case fixedExistingQuantity <= 0:
				quantityChange = "initial"
			case quantity > fixedExistingQuantity+math.Max(slInput.BaseQuantityStep/2, 1e-12):
				quantityChange = "expanded"
			case quantity < fixedExistingQuantity-math.Max(slInput.BaseQuantityStep/2, 1e-12):
				quantityChange = "reduced"
			}
			if quantityChange != "unchanged" {
				key := fmt.Sprintf("%s|%d|%d", quantityChange, protectionSignatureBucket(fixedExistingQuantity), protectionSignatureBucket(quantity))
				if ti.shouldRecordProtectionEvent(cycle.ID, "POSITION_MARGIN_PROTECTION_QUANTITY_SYNCED", key) {
					_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_PROTECTION_QUANTITY_SYNCED", Price: fixedEvaluation.StopPrice, Quantity: quantity, Metadata: map[string]interface{}{"change": quantityChange, "previous_quantity": fixedExistingQuantity, "current_quantity": quantity, "fixed_stop_price": fixedAnchor.Price, "requested_stop_price": fixedEvaluation.StopPrice}})
				}
			}
			if fixedEvaluation.Clamped {
				key := fmt.Sprintf("%d", protectionSignatureBucket(fixedEvaluation.StopPrice))
				if ti.shouldRecordProtectionEvent(cycle.ID, "POSITION_MARGIN_LIQUIDATION_TIGHTENED", key) {
					_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "POSITION_MARGIN_LIQUIDATION_TIGHTENED", Price: fixedEvaluation.StopPrice, Quantity: quantity, Metadata: map[string]interface{}{"anchor_stop_price": fixedAnchor.Price, "requested_stop_price": fixedEvaluation.StopPrice, "liquidation_price": liquidationPrice, "one_way_tightening": true}})
				}
			}
		}
		// Announce the threshold breach on state transitions only; passing
		// exceeded=false also rearms the notice for a later re-crossing.
		if alertStopRisk, tier := ti.shouldAlertStopRiskThreshold(cycle.ID, cycle.ReentryCount, slResult.AccountRiskThresholdExceeded, slResult.ExpectedLossPct, effectiveStopPct); !fixedPositionMargin && alertStopRisk {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
				...
				Metadata: map[string]interface{}{
					...
					"distance_atr_ratio":    slResult.DistanceATRRatio,
					"risk_stop_priority":    cycleConfig.RiskStopPriority,
					"attempt_no":            cycle.ReentryCount,
					"overshoot_tier":        tier,
				},
			})
			if store.ShouldSendCopyGuardEmail(cycleConfig.RiskNotificationLevel, "STOP_RISK_THRESHOLD_EXCEEDED") {
				traderName := ti.traderDisplayName()
				// Keyed on the overshoot tier rather than the raw percentages: those
				// drift with every price tick, which defeated the notifier's dedupe and
				// turned one standing condition into a continuous mail stream. The hour
				// bucket lets the periodic reminder through (notifier dedupe is
				// once-per-process) while still collapsing the poll cadence.
				rateKey := fmt.Sprintf("stop_risk_threshold|%s|%d|%d|%d", ti.traderID, cycle.ID, cycle.ReentryCount, tier)
				dedupKey := fmt.Sprintf("%s|%d", rateKey, time.Now().Unix()/int64(stopRiskAlertReminder.Seconds()))
				notifier.Notify(notifier.Alert{
					Category: "copy_guard", TraderID: ti.traderID, TraderName: traderName,
					Title: fmt.Sprintf("%s | %s 预计止损损失超过预警线", traderName, dec.Symbol),
					Body: fmt.Sprintf("合约: %s\n方向: %s\n预计损失: %.2f USDT（账户 %.2f%%）\n预警线: %.2f%%\n止损距离: %.2f×ATR\n策略: %s；普通跟单目标和 ATR/结构止损未被缩量。\n说明: 此提示不改变止损距离；同一超限状态只在首次越线、超限程度显著加深或每 %.0f 小时提醒一次。",
						dec.Symbol, expectedSide, slResult.ExpectedLossUSD, slResult.ExpectedLossPct*100,
						effectiveStopPct*100, slResult.DistanceATRRatio, cycleConfig.RiskStopPriority,
						stopRiskAlertReminder.Hours()),
					RateKey: rateKey, DedupKey: dedupKey,
				})
			}
		}
	...
	if slResult.SLPrice <= 0 {
		...
		case slResult.UnprotectableStructural:
			cause = fmt.Errorf("%w: distance %.4f ATR below the %.2f ATR floor (leverage=%d liq=%.4f)",
				errUnprotectableStructuralNoise, slResult.DistanceATRRatio, cycleConfig.RiskMinStopATRRatio, leverage, liquidationPrice)
	...
	referenceType := cycleConfig.RiskTriggerPriceType
	if fixedPositionMargin {
		referenceType = "mark"
	}
	current, priceKind := float64(0), ""
	if fixedPositionMargin {
		current, priceKind = ti.fixedStopReferencePrice(dec.Symbol, positionMarkPrice)
	} else {
		current, priceKind = ti.stopTriggerReferencePriceForType(dec.Symbol, positionMarkPrice, referenceType)
	}
	if current > 0 {
		crossed := (expectedSide == "long" && current <= slResult.SLPrice) || (expectedSide == "short" && current >= slResult.SLPrice)
		if crossed {
			if fixedPositionMargin && fixedEvaluation != nil {
				fixedEvaluation.AlreadyCrossed = true
				ti.handlePositionMarginStopCrossed(dec, cycle, expectedSide, quantity, current, fixedEvaluation)
				return
			}
	...
	} else if fixedPositionMargin {
		// The venue-managed MARK_PRICE stop can still be armed and remains the
		// primary protection. Missing mark data only disables the local emergency
		// crossing path; last trade must never substitute for it.
		ti.recordPositionMarginMarkUnavailable(cycle, dec.Symbol, "pre_arm_crossing_check", nil)
	}
	if slResult.LiquidationPriceIgnored {
		logger.Warnf("⚠️ [%s] 强平价方向异常已忽略 | %s %s entry=%.4f liq=%.4f mode=%s", ti.traderID, dec.Symbol, expectedSide, entryPrice, liquidationPrice, protectionMode)
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "LIQ_PRICE_IGNORED", Price: entryPrice, Metadata: map[string]interface{}{"liquidation_price": liquidationPrice, "side": expectedSide, "reason": "direction implausible", "protection_mode": protectionMode}})
	}
	dec.Leverage = leverage
	ti.upsertV4ProtectionWithFollowerPosition(
		dec, expectedSide, quantity, entryPrice, slResult,
		getStringField(matchedPos, "posId", "positionId"), true,
	)
}
...
func (ti *TraderIntegration) enforceAIReentryProtection(dec *decision.Decision) {
	...
	if _, closeErr := ti.closeCopyGuardPosition(cycle); closeErr != nil {
...
func (ti *TraderIntegration) usesAIGuardedLifecycle(dec *decision.Decision) bool {
	if ti == nil || dec == nil || ti.store == nil || ti.engine == nil || ti.engine.config == nil || !isAIReentryDecision(dec) {
		return false
	}
	cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID)
	if err != nil {
	...
	cfg := copyGuardLifecycleConfig(cycle, ti.engine.config)
	return cfg != nil && cfg.RiskProtectionMode != store.RiskProtectionModePositionMarginPct &&
		cfg.RiskReentryEnabled && cfg.RiskReentryDecisionMode == "ai_guarded"
}

// rejectPositionMarginReentryBeforeOrder is the final runtime barrier for a
// stale queued AI/manual/legacy reentry decision. Scheduling and API checks are
// not sufficient: a decision may already be buffered when configuration or a
// process restarts. Fixed first-entry protection never permits a second entry,
// so the immutable cycle snapshot must be checked before any venue call.
func (ti *TraderIntegration) rejectPositionMarginReentryBeforeOrder(dec *decision.Decision) error {
	if ti == nil || dec == nil || !isAIReentryDecision(dec) || ti.store == nil || ti.engine == nil || ti.engine.config == nil || ti.engine.config.RiskPolicyVersion < 4 {
		return nil
	}
	cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, dec.LeaderPosID)
	if err != nil {
		return aiReentryPreflightError("CYCLE_STATE_CHANGED", "reentry lifecycle is unavailable: %v", err)
	}
	cfg := copyGuardLifecycleConfig(cycle, ti.engine.config)
	if cfg == nil {
		return aiReentryPreflightError("EXECUTION_CONTEXT_UNAVAILABLE", "reentry lifecycle policy is unavailable")
	}
	if cfg.RiskProtectionMode == store.RiskProtectionModePositionMarginPct {
		return aiReentryPreflightError("POSITION_MARGIN_REENTRY_BLOCKED", "fixed first-entry position-margin protection forbids every secondary entry")
	}
	return nil
}
...
func (ti *TraderIntegration) upsertV4Protection(dec *decision.Decision, side string, quantity, entryPrice float64, result *StopLossCalcResult) {
	ti.upsertV4ProtectionWithFollowerPosition(dec, side, quantity, entryPrice, result, "", false)
}

// upsertV4ProtectionWithFollowerPosition accepts the identity already proven by
// refreshStopLossAfterExecuteLocked. Direct recovery/test callers retain the
// wrapper above, which fetches identity itself. This avoids a second positions
// request per cycle during the periodic account-wide safety sweep.
func (ti *TraderIntegration) upsertV4ProtectionWithFollowerPosition(dec *decision.Decision, side string, quantity, entryPrice float64, result *StopLossCalcResult, followerPosID string, positionSnapshotKnown bool) {
	...
	cycle, err := ti.protectionCycleForDecision(dec)
	if err != nil {
	...
	conflicts, conflictErr := ti.store.CopyTrade().ProtectionScopeConflicts(cycle.ID)
	if conflictErr != nil || len(conflicts) > 0 {
		if conflictErr == nil {
			conflictErr = fmt.Errorf("multiple protection anchors cover the same position: cycles %v", conflicts)
		}
		ti.recordProtectionFollowup(dec, "PROTECTION_SCOPE_CONFLICT", conflictErr)
		return
	}
	cycleConfig := copyGuardLifecycleConfig(cycle, ti.engine.config)
	if cycleConfig == nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_POLICY_UNAVAILABLE", fmt.Errorf("lifecycle policy unavailable"), 0, incrementRetry)
		return
	}
	triggerType := cycleConfig.RiskTriggerPriceType
	if mode, _ := copyGuardProtectionSettings(cycle, cycleConfig); mode == store.RiskProtectionModePositionMarginPct {
		triggerType = "mark"
	}
	...
	riskPromoted := false
	if usesV4CopyGuardRisk(cycleConfig) && isAIReentryDecision(dec) && dec.ExecutionIntentID > 0 {
		...
		promoted, promoteErr := ti.store.ReentryAI().PromoteCopyGuardIntentRisk(
			...
			result.ExpectedLossUSD, equity, cycleConfig.RiskAccountPct,
			cycleConfig.RiskCycleLossBudgetPct, cycleConfig.RiskPortfolioLossBudgetPct,
			dec.QuantityStepOverride && result.ExpectedLossUSD > equity*cycleConfig.RiskAccountPct+1e-9,
		)
		if promoteErr != nil {
		...
		if promoted && result.ExpectedLossUSD > equity*cycleConfig.RiskAccountPct+1e-9 {
			dec.QuantityStepOverride = true
		}
	}
	if !positionSnapshotKnown {
		followerPosID = ""
		if positions, posErr := ti.getFreshPositions(); posErr == nil {
			for _, pos := range positions {
				if getStringField(pos, "symbol") == dec.Symbol && getStringField(pos, "side") == side && (dec.MarginMode == "" || getStringField(pos, "mgnMode", "marginMode") == dec.MarginMode) {
					followerPosID = getStringField(pos, "posId", "positionId")
					break
				}
			}
		}
	...
	// 固定比例模式已在计算分支通过原子单向更新写入；这里不能再用普通 UPDATE
	// 覆盖，否则跨进程恢复时可能把更安全的 final_stop_price 放宽。
	if mode, _ := copyGuardProtectionSettings(cycle, cycleConfig); mode != store.RiskProtectionModePositionMarginPct {
	...
	coverageMode := trader.ProtectiveStopCoverageExactQuantity
	if provider, ok := ti.executor.(trader.ProtectiveStopCoverageModeProvider); ok {
		if reported := provider.ProtectiveStopCoverageMode(); reported == trader.ProtectiveStopCoverageCloseAll || reported == trader.ProtectiveStopCoverageExactQuantity {
			coverageMode = reported
		}
	}
	clientID, identityErr := ti.store.CopyTrade().CopyGuardProtectionClientID(cycle.ID, cycle.ReentryCount, false)
	if identityErr != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_IDENTITY_PERSIST_FAILED", identityErr, 0, false)
		return
	}
	req := trader.ProtectiveStopRequest{Symbol: dec.Symbol, PositionSide: side, MarginMode: dec.MarginMode, Quantity: quantity, TriggerPrice: result.SLPrice, TriggerType: triggerType, ClientID: clientID, CoverageMode: coverageMode}
	...
	if storedErr == nil && stored.AlgoID != "" && stored.Status == "live" {
		...
		case resolveErr != nil:
			...
			ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_VERIFY_UNKNOWN", resolveErr, protectiveOrderCoverage(stored.CoverageMode, stored.Quantity, quantity), incrementRetry)
			return
		case actual == nil:
		...
		case isProtectiveStopFired(actual.State):
			ti.observeTrustedProtectiveTrigger(cycle, stored, actual)
		...
		case strings.EqualFold(actual.State, "live"):
			if actual.CoverageMode == trader.ProtectiveStopCoverageCloseAll {
				coverageMode, req.CoverageMode = actual.CoverageMode, actual.CoverageMode
			}
			closeAllIdentityMatches := coverageMode == trader.ProtectiveStopCoverageCloseAll &&
				actual.CoverageMode == trader.ProtectiveStopCoverageCloseAll &&
				strings.EqualFold(actual.PositionSide, side) &&
				protectiveMarginScopeMatches(actual, dec.MarginMode) &&
				strings.EqualFold(actual.TriggerType, triggerType) &&
				sameProtectivePrice(actual.TriggerPrice, result.SLPrice, result.TickSize)
			if closeAllIdentityMatches {
				// Binance closePosition=true follows the whole side automatically.
				// Quantity changes only update our audit ledger; replacing this live
				// order would create a needless unprotected retirement window.
				actual.Quantity = quantity
				live, reuseCloseAll = actual, true
			} else {
	...
	if storedErr == nil {
		previousPrice = stored.TriggerPrice
	}
	if amendExisting != nil {
		previousPrice = amendExisting.TriggerPrice
	}
	if err := ti.store.CopyTrade().PrepareCopyGuardStopRequest(cycle.ID, cycle.ReentryCount, req.TriggerPrice, previousPrice); err != nil {
		ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_REQUEST_PERSIST_FAILED", err, 0, false)
		return
	}
	if !ti.copyGuardOwnsPosition(cycle) {
		return
	...
	if reuseCloseAll {
		// Exchange mutation intentionally skipped; common verification below
		// still proves the order remains live and persists the new audit quantity.
	} else if amendAlgoID != "" {
			...
			if ensured != nil {
				...
				if live != nil && ensured.ReplacementPending && ensured.Retiring != nil {
					pending := &store.CopyGuardProtectiveOrder{
						...
						QuantityStep: result.QuantityStep, CoverageMode: coverageMode, TriggerPrice: live.TriggerPrice, TriggerType: triggerType,
		...
		if err != nil && trader.IsOKXAlgoAlreadyExists(err) {
			...
			case adopted == nil:
				...
				req.ClientID, err = ti.store.CopyTrade().CopyGuardProtectionClientID(cycle.ID, cycle.ReentryCount, true)
				if err != nil {
					ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "PROTECTION_IDENTITY_PERSIST_FAILED", err, 0, false)
					return
				}
				live, err = mgr.PlaceProtectiveStop(req)
	...
	if err != nil {
		coverage := 0.0
		if storedErr == nil {
			coverage = protectiveOrderCoverage(stored.CoverageMode, stored.Quantity, quantity)
		}
		...
		if trader.IsProtectiveStopAlreadyTriggerable(err) {
			if mode, _ := copyGuardProtectionSettings(cycle, cycleConfig); mode == store.RiskProtectionModePositionMarginPct {
				current, _ := ti.fixedStopReferencePrice(dec.Symbol, 0)
				if current <= 0 {
					ti.recordPositionMarginMarkUnavailable(cycle, dec.Symbol, "venue_rejected_already_triggerable", err)
					ti.markProtectionIssue(cycle, store.CopyGuardProtectionUnknown, "POSITION_MARGIN_MARK_UNAVAILABLE", err, coverage, incrementRetry)
					return
				}
				ti.handlePositionMarginStopCrossed(dec, cycle, side, quantity, current, &PositionMarginStopEvaluation{
					StopPrice: result.SLPrice, CurrentRiskUSD: result.ExpectedLossUSD,
					CurrentEffectiveMarginLossPct: result.ExpectedMarginLossPct,
					DistanceATRRatio:              result.DistanceATRRatio, Clamped: result.Clamped,
					AlreadyCrossed: true, GovernedBy: result.GovernedBy,
				})
				return
			}
	...
	verified, verifyErr := ti.verifyProtectiveStopWithGrace(
		...
		quantity, result.SLPrice, result.QuantityStep, result.TickSize, coverageMode, triggerType,
	)
	if verifyErr != nil {
	...
	if !ti.copyGuardOwnsPosition(cycle) {
		_ = ti.cancelProtectiveOrderForCycle(mgr, cycle, &store.CopyGuardProtectiveOrder{CycleID: cycle.ID, AlgoID: verified.AlgoID, Symbol: dec.Symbol})
		return
	}
	if sameProtectivePrice(verified.TriggerPrice, req.TriggerPrice, result.TickSize) {
		_ = ti.store.CopyTrade().ConfirmCopyGuardStopRequest(cycle.ID, cycle.ReentryCount, req.TriggerPrice)
	}
	verifiedCoverageMode := verified.CoverageMode
	if verifiedCoverageMode == "" {
		verifiedCoverageMode = coverageMode
	}
	coverage := protectiveOrderCoverage(verifiedCoverageMode, verified.Quantity, quantity)
	if isProtectiveStopFired(verified.State) {
		ti.observeTrustedProtectiveTrigger(cycle, &store.CopyGuardProtectiveOrder{TriggerPrice: result.SLPrice, Quantity: quantity}, verified)
		return
	}
	valid := protectiveTriggerMatches(verified, dec.Symbol, triggerType) && strings.EqualFold(verified.PositionSide, side) && protectiveMarginScopeMatches(verified, dec.MarginMode) && sameProtectivePrice(verified.TriggerPrice, result.SLPrice, result.TickSize) && verifiedCoverageMode == coverageMode && protectiveOrderQuantityMatches(verifiedCoverageMode, verified.Quantity, quantity, result.QuantityStep) && strings.EqualFold(verified.State, "live")
	if !valid {
		// Record what actually exists on OKX (not the requested quantity) so
		// local tracking stays truthful while retries converge.
		_ = ti.store.CopyTrade().UpsertCopyGuardProtectiveOrder(&store.CopyGuardProtectiveOrder{CycleID: cycle.ID, TraderID: ti.traderID, AlgoID: verified.AlgoID, AlgoClientID: verified.ClientID, Symbol: verified.Symbol, Side: verified.PositionSide, MarginMode: verified.MarginMode, Quantity: verified.Quantity, QuantityStep: result.QuantityStep, CoverageMode: verifiedCoverageMode, TriggerPrice: verified.TriggerPrice, TriggerType: verified.TriggerType, Status: strings.ToLower(verified.State)})
	...
	auditQuantity := verified.Quantity
	if verifiedCoverageMode == trader.ProtectiveStopCoverageCloseAll {
		auditQuantity = quantity
	}
	_ = ti.store.CopyTrade().UpsertCopyGuardProtectiveOrder(&store.CopyGuardProtectiveOrder{CycleID: cycle.ID, TraderID: ti.traderID, AlgoID: verified.AlgoID, AlgoClientID: verified.ClientID, Symbol: verified.Symbol, Side: verified.PositionSide, MarginMode: verified.MarginMode, Quantity: auditQuantity, QuantityStep: result.QuantityStep, CoverageMode: verifiedCoverageMode, TriggerPrice: verified.TriggerPrice, TriggerType: verified.TriggerType, Status: "live"})
	if usesV4CopyGuardRisk(cycleConfig) && isAIReentryDecision(dec) && !riskPromoted {
		equity := cycle.AccountEquity
		if equity <= 0 {
			equity = ti.getEquityFunc()()
		}
		var reserveErr error
		if dec.QuantityStepOverride && result.ExpectedLossUSD > equity*cycleConfig.RiskAccountPct+1e-9 {
			reserveErr = ti.store.ReentryAI().ReserveCopyGuardRiskFollowFirst(ti.traderID, cycle.ID, cycle.ReentryCount, result.ExpectedLossUSD, equity, cycleConfig.RiskAccountPct, cycleConfig.RiskCycleLossBudgetPct, cycleConfig.RiskPortfolioLossBudgetPct)
		} else {
			reserveErr = ti.store.ReentryAI().ReserveCopyGuardRisk(ti.traderID, cycle.ID, cycle.ReentryCount, result.ExpectedLossUSD, equity, cycleConfig.RiskAccountPct, cycleConfig.RiskCycleLossBudgetPct, cycleConfig.RiskPortfolioLossBudgetPct)
		}
		if reserveErr != nil {
		...
		if dec.QuantityStepOverride && result.ExpectedLossUSD > equity*cycleConfig.RiskAccountPct+1e-9 {
			_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{CycleID: cycle.ID, TraderID: ti.traderID, Type: "RISK_STEP_OVERRIDE_ACCEPTED", Price: entryPrice, Quantity: quantity, Notional: entryPrice * quantity, Metadata: map[string]interface{}{"attempt_no": cycle.ReentryCount, "expected_loss_usd": result.ExpectedLossUSD, "attempt_budget_usd": equity * cycleConfig.RiskAccountPct, "cycle_hard_budget_usd": equity * cycleConfig.RiskCycleLossBudgetPct, "portfolio_hard_budget_usd": equity * cycleConfig.RiskPortfolioLossBudgetPct}})
		}
	}
	...
	// Throttled: the retry chain re-arms and re-verifies on every poll, so an
	// unstable cycle used to write one PROTECTION_ACTIVE row every 3 seconds. A
	// materially different order (new algo id, quantity, or trigger) still records
	// immediately.
	activeSignature := fmt.Sprintf("%s|%d|%d", live.AlgoID, protectionSignatureBucket(verified.Quantity), protectionSignatureBucket(result.SLPrice))
	if ti.shouldRecordProtectionEvent(cycle.ID, "PROTECTION_ACTIVE", activeSignature) {
	...
	if cycleConfig.RiskReentryDecisionMode == "ai_guarded" && cycle.Status == store.CopyGuardFollowingReentry {
		ti.enforceAIReentryProtection(dec)
	}
	if result.Clamped {
...
// verifyProtectiveStopWithGrace waits for the requested target, not only for a
// successful lookup. Exact-quantity venues must settle both quantity and price;
// CLOSE_ALL venues settle by identity and price because the exchange follows the
// live side quantity automatically. OKX may briefly return the pre-amend quantity or
...
func (ti *TraderIntegration) verifyProtectiveStopWithGrace(
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		if err == nil {
			if verified != nil && isProtectiveStopFired(verified.State) {
				// A trigger is not an unsettled amend. Return it immediately so
				// the caller installs the risk gate before any further I/O.
				return verified, nil
			}
			lastObserved = verified
			verifiedCoverageMode := coverageMode
			if verified != nil && verified.CoverageMode != "" {
				verifiedCoverageMode = verified.CoverageMode
			}
			if verified != nil &&
				(len(triggerTypes) == 0 || protectiveTriggerMatches(verified, symbol, triggerTypes[0])) &&
				strings.EqualFold(verified.PositionSide, side) &&
				protectiveMarginScopeMatches(verified, marginMode) &&
				sameProtectivePrice(verified.TriggerPrice, triggerPrice, tickSize) &&
				protectiveOrderQuantityMatches(verifiedCoverageMode, verified.Quantity, quantity, quantityStep) &&
...
func (ti *TraderIntegration) retryRetiringProtectiveStop(mgr ProtectiveStopManagerV4, cycle *store.CopyGuardCycle, stored *store.CopyGuardProtectiveOrder) bool {
	...
	if current == nil || !strings.EqualFold(current.State, "live") {
		...
		if old != nil && strings.EqualFold(old.State, "live") {
			coverageMode := old.CoverageMode
			if coverageMode == "" {
				coverageMode = stored.CoverageMode
			}
			quantity := old.Quantity
			if coverageMode == trader.ProtectiveStopCoverageCloseAll || quantity <= 0 {
			...
			if restoreErr := ti.store.CopyTrade().UpsertCopyGuardProtectiveOrder(&store.CopyGuardProtectiveOrder{
				...
				QuantityStep: stored.QuantityStep, CoverageMode: coverageMode, TriggerPrice: triggerPrice, TriggerType: stored.TriggerType, Status: "live",
			}); restoreErr != nil {
...
func (ti *TraderIntegration) adoptProtectiveOrderByClientID(mgr ProtectiveStopManagerV4, cycle *store.CopyGuardCycle, req trader.ProtectiveStopRequest, tickSize, quantityStep float64) (*trader.ProtectiveStopOrder, error) {
	...
	if existing == nil {
		return nil, fmt.Errorf("conflicting client identity has no confirmed terminal order")
	}
	...
	if state != "live" && !isProtectiveStopFired(existing.State) {
		...
		return nil, fmt.Errorf("conflicting protective order state is unknown: %s", state)
	}
	if isProtectiveStopFired(existing.State) {
		ti.observeTrustedProtectiveTrigger(cycle, &store.CopyGuardProtectiveOrder{TriggerPrice: req.TriggerPrice, Quantity: req.Quantity}, existing)
	}
	...
	existingCoverageMode := existing.CoverageMode
	if existingCoverageMode == "" {
		existingCoverageMode = req.CoverageMode
	}
	_ = ti.store.CopyTrade().UpsertCopyGuardProtectiveOrder(&store.CopyGuardProtectiveOrder{CycleID: cycle.ID, TraderID: ti.traderID, AlgoID: existing.AlgoID, AlgoClientID: existing.ClientID, Symbol: existing.Symbol, Side: existing.PositionSide, MarginMode: existing.MarginMode, Quantity: existing.Quantity, QuantityStep: quantityStep, CoverageMode: existingCoverageMode, TriggerPrice: existing.TriggerPrice, TriggerType: existing.TriggerType, Status: strings.ToLower(existing.State)})
	...
	priceOK := sameProtectivePrice(existing.TriggerPrice, req.TriggerPrice, tickSize) && protectiveTriggerMatches(existing, req.Symbol, req.TriggerType)
	if !priceOK || !protectiveOrderQuantityMatches(existingCoverageMode, existing.Quantity, req.Quantity, quantityStep) {
		if amendErr := mgr.AmendProtectiveStop(existing.AlgoID, req); amendErr != nil {
			return nil, amendErr
		}
	}
	return &trader.ProtectiveStopOrder{AlgoID: existing.AlgoID, ClientID: existing.ClientID, Symbol: req.Symbol, PositionSide: req.PositionSide, MarginMode: req.MarginMode, Quantity: req.Quantity, TriggerPrice: req.TriggerPrice, TriggerType: req.TriggerType, CoverageMode: existingCoverageMode, State: "live"}, nil
}

// followerPositionQuantity returns the follower position size attributable to
// one protection scope (symbol + side + marginMode, narrowed by posID when the
// caller knows it). ok is false when the size cannot be determined; callers must
// treat that as unknown, never as flat and never as insufficient coverage.
//
// Attribution is load-bearing. A leader running two positions on the same
// symbol/side (e.g. one cross, one isolated) produces two independent Copy Guard
// cycles, each protecting only its own slice. Measuring one slice against the
// account-wide total makes coverage permanently irreconcilable and would let one
// cycle's residual close reach into the other cycle's position. This is what a
// dropped mgnMode field caused in production: the margin-mode filter below
// silently matched everything and summed cross+isolated into one baseline.
//
// fresh 语义：
...
//     限流（跟单执行失败）的根因之一。
func (ti *TraderIntegration) followerPositionQuantity(symbol, side, marginMode, posID string, fresh bool) (float64, bool) {
	var positions []map[string]interface{}
	var err error
	if fresh {
		positions, err = ti.getFreshPositions()
	...
	posID = strings.TrimSpace(posID)
	scopedQty, scopedFound := 0.0, false
	total := 0.0
	matched := 0
	modeUnreported := false
	for _, pos := range positions {
		...
		amount, known := knownQuantityField(pos, "positionAmt", "quantity")
		if !known {
			return 0, false
		}
		qty := absFloat(amount)
		if posID != "" && getStringField(pos, "posId", "positionId") == posID {
			scopedQty, scopedFound = qty, true
		}
		if marginMode != "" {
			mode := getStringField(pos, "mgnMode", "marginMode")
			if mode == "" {
			...
			if !strings.EqualFold(mode, marginMode) {
				continue
			}
		}
		matched++
	...
	// Exchange-assigned position identity is the strongest signal available.
	if scopedFound {
		return scopedQty, true
	}
	// A scope we were asked to honour but cannot evaluate: fail closed rather
	// than fall back to an account-wide sum.
	if modeUnreported {
		return 0, false
	}
	// Several distinct positions share the scope and nothing tells them apart.
	if matched > 1 {
		return 0, false
	}
	return total, true
...
// position for the given protection scope. Returns false when positions cannot
// be fetched or attributed (unknown must not be treated as flat).
func (ti *TraderIntegration) isFollowerPositionFlat(symbol, side, marginMode, posID string) bool {
	// 空仓判定必须用最新数据：结果直接决定"止损已触发"与"保护降级"的分流
	qty, ok := ti.followerPositionQuantity(symbol, side, marginMode, posID, true)
	return ok && qty == 0
}
	...
	terminalCancel := func(algoID string, requireFlatForMissing bool) error {
		...
		if err == nil {
			...
			for attempt := 0; attempt < 3; attempt++ {
				...
				if errors.Is(queryErr, trader.ErrProtectiveStopNotFound) || (queryErr == nil && live == nil) {
					return nil
				}
				if queryErr != nil {
					if attempt == 2 {
				...
				if cycle.ClosedAt == nil && isProtectiveStopFired(lastState) {
					ti.observeTrustedProtectiveTrigger(cycle, order, live)
				}
				switch lastState {
				case "canceled", "cancelled", "order_failed", "failed", "expired":
					return nil
				case "filled", "triggered", "effective":
					if !requireFlatForMissing || cycle.ClosedAt != nil || ti.isFollowerPositionFlat(cycle.Symbol, cycle.Side, cycle.MarginMode, cycle.FollowerPosID) {
						return nil
					}
				}
				time.Sleep(100 * time.Millisecond)
...
func (ti *TraderIntegration) markProtectionIssueForDecision(dec *decision.Decision, status, eventType string, cause error, coverage float64) {
	...
	if !isAIReentryDecision(dec) {
		// Protection health must not change the already committed leader fill or
		// its independent partial-fill/catch-up state.
		return
	}
	ti.transitionExecutionIntent(dec, store.ExecutionIntentReconciling, eventType, message)
}
...
func (ti *TraderIntegration) markProtectionIssue(cycle *store.CopyGuardCycle, status, eventType string, cause error, coverage float64, incrementRetry bool) {
	...
	if err != nil || current.ClosedAt != nil || current.ReentryCount != cycle.ReentryCount || current.ProtectionStatus == store.CopyGuardProtectionFlatReconciling {
		return
	}
	message := "unknown protection error"
	if cause != nil {
...
type stopRiskAlertState struct {
	// tier is the deepest overshoot band reported so far; a shallower reading is
	// an improvement and must not re-alert.
	tier       int
	notifiedAt time.Time
}

const (
	// stopRiskAlertTierStep quantizes the overshoot (expected loss ÷ warning line)
	// into 25% bands.
	stopRiskAlertTierStep = 0.25

	// stopRiskAlertReminder re-states the warning while the condition persists, so
	// a long-lived position is not silently forgotten.
	stopRiskAlertReminder = 6 * time.Hour
)

// shouldAlertStopRiskThreshold reports whether "expected stop loss exceeds the
// account warning line" warrants a fresh notice for this cycle attempt, and
// returns the overshoot tier to key that notice on.
//
// This condition is a state, not an event: it holds for as long as the position
// is open and is recomputed from live price/ATR on every 3s poll. Under the
// default volatility_first priority it does not change the stop distance at all,
// so it is purely a heads-up. Announcing it per evaluation produced 698 events and
// a matching stream of emails in 38 minutes. Instead it is announced when first
// crossed, again only when the overshoot deepens by a full tier or the reminder
// interval elapses, and rearmed once the condition clears.
func (ti *TraderIntegration) shouldAlertStopRiskThreshold(cycleID int64, attemptNo int, exceeded bool, expectedLossPct, thresholdPct float64) (bool, int) {
	key := fmt.Sprintf("%d|%d", cycleID, attemptNo)
	ti.protectionEventMu.Lock()
	defer ti.protectionEventMu.Unlock()
	if !exceeded {
		delete(ti.stopRiskAlerts, key)
		return false, 0
	}
	tier := 0
	if thresholdPct > 0 && expectedLossPct > 0 {
		tier = int(math.Floor(expectedLossPct / thresholdPct / stopRiskAlertTierStep))
	}
	now := time.Now()
	state, tracked := ti.stopRiskAlerts[key]
	if tracked && tier <= state.tier && now.Sub(state.notifiedAt) < stopRiskAlertReminder {
		return false, state.tier
	}
	if !tracked {
		state = &stopRiskAlertState{}
		if len(ti.stopRiskAlerts) > 512 {
			for k, v := range ti.stopRiskAlerts {
				if now.Sub(v.notifiedAt) > 24*time.Hour {
					delete(ti.stopRiskAlerts, k)
				}
			}
		}
		ti.stopRiskAlerts[key] = state
	}
	if tier > state.tier {
		state.tier = tier
	}
	state.notifiedAt = now
	return true, state.tier
}

// protectionSignatureBucket quantizes a price or quantity into a scale-free
// bucket (~0.1% wide) for de-duplication keys.
//
// Raw floats must never be used as dedupe keys: a stop price recomputed from live
// ATR drifts on every poll, so the key changes every time and the de-duplication
// silently degrades to "record everything" — which is exactly how one cycle wrote
// hundreds of identical events and emails in production.
func protectionSignatureBucket(v float64) int64 {
	if v == 0 || math.IsNaN(v) || math.IsInf(v, 0) {
		return 0
	}
	return int64(math.Round(math.Log(math.Abs(v)) / math.Log(1.001)))
}
...
// notifyProtection sends a protection alert unconditionally. Every remaining
// caller reports a condition that can lose capital (naked position, forced exit,
// ledger commit failure), so these are treated as critical regardless of the
// configured email level.
func (ti *TraderIntegration) notifyProtection(cycle *store.CopyGuardCycle, title, body, kind string) {
	ti.notifyProtectionForEvent(cycle, title, body, kind, "")
}

// notifyProtectionForEvent gates the alert on the trader's configured email level
// using the event type's declared level in copyGuardEventSpecs. Passing an empty
// eventType always sends.
//
// Without this, "邮件级别" was a setting with no effect on protection alerts: the
// class of alert most likely to repeat was also the class that ignored the
// setting.
func (ti *TraderIntegration) notifyProtectionForEvent(cycle *store.CopyGuardCycle, title, body, kind, eventType string) {
	if eventType != "" {
		level := ""
		if ti.engine != nil && ti.engine.config != nil {
			level = ti.engine.config.RiskNotificationLevel
		}
		if !store.ShouldSendCopyGuardEmail(level, eventType) {
			return
		}
	}
	...
	traderName := ti.traderDisplayName()
	alert := notifier.Alert{Category: "copy_trade", TraderID: ti.traderID, TraderName: traderName, Title: fmt.Sprintf("%s | %s | %s %s", traderName, title, cycle.Symbol, cycle.Side), Body: fmt.Sprintf("Trader Name: %s\nTrader ID:   %s\n\n%s", traderName, ti.traderID, body), RateKey: key, DedupKey: dedupKey, Fields: map[string]string{"TraderName": traderName, "CycleID": fmt.Sprint(cycle.ID), "LeaderPosID": cycle.LeaderPosID, "Symbol": cycle.Symbol, "Side": cycle.Side, "MarginMode": cycle.MarginMode}}
	if ti.attachProtectionIncident(&alert, cycle, kind) {
...
func (ti *TraderIntegration) lifecycleConfigForRiskEvent(event *RiskEvent) (*CopyConfig, *store.CopyGuardCycle) {
	if ti == nil || ti.store == nil || ti.engine == nil || ti.engine.config == nil || event == nil {
		return nil, nil
	}
	var cycle *store.CopyGuardCycle
	var err error
	if event.CycleID > 0 {
		cycle, err = ti.store.CopyTrade().GetCopyGuardCycle(event.CycleID)
	} else if event.LeaderPosID != "" {
		cycle, err = ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, event.LeaderPosID)
	}
	if err != nil || cycle == nil {
		return ti.engine.config, nil
	}
	return copyGuardLifecycleConfig(cycle, ti.engine.config), cycle
}

func (ti *TraderIntegration) sendStopLossTriggerAlert(event *RiskEvent) {
	lifecycleConfig, lifecycleCycle := ti.lifecycleConfigForRiskEvent(event)
	if lifecycleConfig != nil && !store.ShouldSendCopyGuardEmail(lifecycleConfig.RiskNotificationLevel, "STOP_FLAT_CONFIRMED") {
		return
	}
	traderName := ti.traderDisplayName()
	providerType := ""
	...
	if lifecycleCycle != nil {
		cycleID, attemptNo = lifecycleCycle.ID, lifecycleCycle.ReentryCount
	} else if cycleID == 0 {
	...
	if lifecycleConfig != nil && lifecycleConfig.RiskReentryEnabled {
		if lifecycleConfig.RiskReentryDecisionMode == "ai_guarded" {
			...
			recoveryHint = fmt.Sprintf(
				...
				lifecycleConfig.RiskReentryRatio*100)
		}
	}
...
func (ti *TraderIntegration) sendReentryInitiatedAlert(event *RiskEvent) {
	lifecycleConfig, lifecycleCycle := ti.lifecycleConfigForRiskEvent(event)
	if lifecycleConfig != nil && lifecycleConfig.RiskReentryDecisionMode == "ai_guarded" {
		return
	}
	traderName := ti.traderDisplayName()
	providerType := ""
	...
	if ti.engine != nil && ti.engine.config != nil {
		...
		if lifecycleConfig != nil {
			reentryRatio = lifecycleConfig.RiskReentryRatio
		} else {
	...
	if lifecycleCycle != nil {
		cycleID, attemptNo = lifecycleCycle.ID, lifecycleCycle.ReentryCount+1
	} else if cycleID == 0 {
		if cycle, err := ti.store.CopyTrade().GetOpenCopyGuardCycle(ti.traderID, event.LeaderPosID); err == nil {
...
func (ti *TraderIntegration) ExecuteAIReentry(candidateID, analysisID int64) error {
	...
	if _, persistedErr := ti.store.CopyTrade().GetByTraderID(ti.traderID); persistedErr != nil {
		return reasonError("EXECUTION_CONTEXT_UNAVAILABLE", "authoritative copy configuration is unavailable")
	}
	aiCfg, err := ti.store.ReentryAI().GetReentryAIConfig()
	if err != nil {
	...
	snapshotCycle, cycleErr := ti.store.CopyTrade().GetCopyGuardCycle(candidate.CycleID)
	if cycleErr != nil {
		return fmt.Errorf("Copy Guard lifecycle unavailable: %w", cycleErr)
	}
	cfg := copyGuardLifecycleConfig(snapshotCycle, ti.engine.config)
	if cfg == nil || cfg.RiskProtectionMode == store.RiskProtectionModePositionMarginPct {
		_ = rs.MarkReentryCandidateStatus(candidate.ID, store.ReentryCandidateInvalidated, "position_margin_pct forbids reentry")
		return reasonError("EXECUTION_DISABLED", "position_margin_pct forbids AI and secondary entry")
	}
	if cfg.RiskReentryDecisionMode != "ai_guarded" || !cfg.RiskReentryEnabled {
		return reasonError("EXECUTION_DISABLED", "lifecycle policy does not enable AI guarded reentry")
	}
	...
	if !ti.engine.emitReentryDecision(mapping, leader, notional, price, intent, sizing, cfg) {
		if replayingLease {
...
// healStoppedProtectionBookkeeping clears "filled but never recorded as protected"
// blockers by re-deriving the fact from the exchange.
//
...
// leaves the blocker in place.
func (ti *TraderIntegration) healStoppedProtectionBookkeeping(blockers []store.TraderLifecycleBlocker) bool {
	mgr, ok := ti.executor.(ProtectiveStopManagerV4)
	if !ok {
		return false
	}
	...
	for _, blocker := range blockers {
		...
		intent, intentErr := ti.store.CopyTrade().GetExecutionIntentByID(intentID)
		if intentErr != nil || intent.CycleID <= 0 || intent.ProtectedAt != nil {
			continue
		}
		cycle, cycleErr := ti.store.CopyTrade().GetCopyGuardCycle(intent.CycleID)
		if cycleErr != nil || cycle.ClosedAt != nil {
			continue
		}
		stored, storedErr := ti.store.CopyTrade().GetCopyGuardProtectiveOrder(cycle.ID)
		if storedErr != nil {
		...
		live, liveErr := ti.resolveProtectiveOrder(mgr, stored.AlgoID, stored.AlgoClientID, stored.Symbol)
		if liveErr != nil || live == nil || !strings.EqualFold(live.State, "live") {
			continue
		}
		posQty, attributed := ti.copyGuardFollowerQuantity(cycle, true)
		if !attributed || posQty <= 0 || !protectionQuantityMatches(live.Quantity, posQty, stored.QuantityStep) {
			continue
		}
		if updateErr := ti.store.CopyTrade().UpdateExecutionIntent(intent.ID, store.ExecutionIntentProtected,
			"STOP_RECONCILE_PROTECTION_VERIFIED", "", live.AlgoID, 0, 0, 0); updateErr != nil {
			logger.Warnf("⚠️ [%s] 停机对账补写保护记账失败 | intent=%d cycle=%d: %v", ti.traderID, intent.ID, cycle.ID, updateErr)
			continue
		}
		logger.Infof("🔧 [%s] 停机对账已确认交易所保护单有效并补写记账 | intent=%d cycle=%d algoId=%s qty=%.8f",
			ti.traderID, intent.ID, cycle.ID, live.AlgoID, live.Quantity)
		_ = ti.store.CopyTrade().SaveCopyGuardEvent(&store.CopyGuardEvent{
			CycleID: cycle.ID, TraderID: ti.traderID, Type: "PROTECTION_BOOKKEEPING_HEALED",
			Price: live.TriggerPrice, Quantity: live.Quantity,
			Metadata: map[string]interface{}{"intent_id": intent.ID, "algo_id": live.AlgoID, "position_quantity": posQty},
		})
		healed = true
	}
	return healed
