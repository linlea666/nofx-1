...
func TestExportRiskCycleUsesTraderNameAndOwnership(t *testing.T) {
	...
	// schema_version 7 必须带普通执行意图、完整 AI 候选/分析、后验评价、影子策略和默认值代次。
	if !strings.Contains(recorder.Body.String(), "156-平凡无奇交易员") || !strings.Contains(recorder.Body.String(), `"schema_version":7`) || !strings.Contains(recorder.Body.String(), `"execution_intents":[]`) || !strings.Contains(recorder.Body.String(), `"watch_samples"`) || !strings.Contains(recorder.Body.String(), `"ai_candidates"`) || !strings.Contains(recorder.Body.String(), `"ai_analyses"`) || !strings.Contains(recorder.Body.String(), `"ai_decision_evaluations"`) || !strings.Contains(recorder.Body.String(), `"ai_effect_summary"`) || !strings.Contains(recorder.Body.String(), `"shadow_evaluations"`) || !strings.Contains(recorder.Body.String(), `"defaults_version":8`) {
