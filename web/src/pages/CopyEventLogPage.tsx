...
const eventTypeLabels: Record<string, string> = {
  ...
  STOP_TRIGGERED: '止损触发',
  STOP_CONFIRMED: '止损确认',
  STOP_PENDING_FLAT: '止损待确认平仓',
  STOP_PARTIAL: '止损部分成交',
  STOP_FLAT_CONFIRMED: '止损平仓确认',
  STOP_DUST_RESIDUAL: '止损残留粉尘',
  STOP_RISK_THRESHOLD_EXCEEDED: '预计止损损失超预警线',
  FORCED_EXIT: '强制离场',
  REENTRY_REQUESTED: '二次入场触发',
  REENTRY_SUBMITTED: '二次入场已提交',
  REENTRY_FILLED: '二次入场成交',
  REENTRY_FILL_INCREMENT: '二次入场新增成交',
  REENTRY_RECOVERED_AFTER_RESTART: '二次入场恢复',
  REENTRY_RECOVERY_PENDING: '二次入场状态待确认',
  REENTRY_PREFLIGHT_REJECTED: '二次入场校验未通过',
  REENTRY_FAILED: '二次入场失败',
  REENTRY_WINDOW_COLLAPSED: '旧规则窗口关闭',
  GUARD_MANUAL_REENTRY_SIGNAL: '人工重入信号（历史）',
  GUARD_MANUAL_REENTRY_CONFIRMED: '重入已确认（历史）',
  GUARD_MANUAL_REENTRY_DISMISSED: '重入已忽略（历史）',
  AI_ANALYSIS: 'AI 分析',
  AI_CANDIDATE_CREATED: 'AI 候选建立',
  AI_CANDIDATE_UNACTIONABLE: 'AI 候选不可执行',
  AI_CANDIDATE_TERMINATED: 'AI 候选终结',
  AI_REVIEW_REQUESTED: 'AI 复查请求',
  AI_REVIEW_WAIT: 'AI 裁决·等待',
  AI_REVIEW_ENTER: 'AI 裁决·入场',
  AI_REVIEW_ABANDON: 'AI 裁决·放弃',
  AI_REVIEW_THESIS_INVALID: 'AI 论点失效·休眠',
  AI_CLOSE_INVALIDATION_HIT: 'AI 收盘失效条件命中·仅记录',
  AI_REVIEW_FAILED: 'AI 裁决失败',
  AI_RESULT_STALE: 'AI 结论过期',
  AI_BUDGET_SUSPENDED: 'AI 预算暂停',
  AI_ENTRY_LEASE_WAITING_PRICE: 'AI 租约等待价格',
  AI_DECISION_OUTCOME_FINALIZED: 'AI 决策评价完成',
  AI_CANDIDATE_OUTCOME_FINALIZED: 'AI 候选评价完成',
  ENTER_WINDOW_EXPIRED: 'AI 入场租约到期',
  CATCHUP_TIMEOUT: '跟单补齐超时',
  CATCHUP_PRICE_LIMIT: '跟单补齐触发价格上限',
  PROTECTION_PENDING: '保护建立中',
  PROTECTION_ACTIVE: '保护生效',
  PROTECTIVE_STOP_ACTIVE: '保护生效（历史事件名）',
  PROTECTION_RECOVERED: '保护恢复',
  PROTECTION_DEGRADED: '保护异常',
  PROTECTION_CLAMPED: '止损钳紧',
  PROTECTION_COVERAGE_LOW: '保护覆盖不足',
  PROTECTION_COVERAGE_UNATTRIBUTED: '保护覆盖无法归属',
  PROTECTION_RETRY: '保护重挂中',
  PROTECTION_RETRY_THROTTLED: '保护重挂已转慢速',
  PROTECTION_BOOKKEEPING_HEALED: '保护记账已补齐',
  ...
  ADDON_RISK_WARNING: '加仓风险预警（历史）',
  ACCOUNTING_RECONCILED: '对账完成',
  ATTEMPT_RECONCILED: '单次尝试对账完成',
  WATCH_SUMMARY: '观察期收尾统计',
  CYCLE_CLOSED_SUMMARY: '周期收尾统计',
  CYCLE_SUMMARY_EMAIL_QUEUED: '周期汇总邮件入队',
  CYCLE_SUMMARY_EMAIL_SENT: '周期汇总邮件已发送',
  CYCLE_SUMMARY_EMAIL_FAILED: '周期汇总邮件失败',
  CYCLE_SUMMARY_EMAIL_DEDUPED: '周期汇总邮件已去重',
